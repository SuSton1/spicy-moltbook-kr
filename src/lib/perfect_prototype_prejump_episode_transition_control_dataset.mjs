...
const buildDateSummary = (dateKey, rows = []) => {
  ...
  return {
    ...
    negativeRowCount: rows.filter((row) => row?.outcomeHitTarget !== true).length,
    foldIds: Array.from(
...
const dateSummaryDistance = (left, right) => {
  ...
  return (
    ...
    Math.abs(Number(safeLeft.positiveRowCount ?? 0) - Number(safeRight.positiveRowCount ?? 0)) +
...
const sortNegativeCandidates = (rows = []) =>
...
export const buildPerfectPrototypePrejumpEpisodeTransitionControlDataset = ({
  ...
  for (const [dateKey, rows] of trainByDate.entries()) {
    ...
    if ((summary?.positiveRowCount ?? 0) > 0) positiveDateSummaries.push(summary)
  }
  ...
  const globalFailurePool = sortNegativeCandidates(fitTrainRows)
  ...
  for (const [dateKey, rows] of trainByDate.entries()) {
    ...
    const sameDateNegatives = sortNegativeCandidates(
      rows.filter((row) => row?.rowKey !== canonicalPositive.rowKey),
    ...
    const nearestPositiveDates = positiveDateSummaries
      .filter((summary) => String(summary?.dateKey ?? "").trim() !== dateKey)
      .map((summary) => ({
        summary,
      ...
      .sort(
        (left, right) =>
          ...
          String(left?.summary?.dateKey ?? "").localeCompare(String(right?.summary?.dateKey ?? "")),
      )
      .map((entry) => entry.summary?.dateKey)
      .filter(Boolean)
    const matchedControlRows = uniqueRowsByKey(
      nearestPositiveDates
        .flatMap((candidateDateKey) => sortNegativeCandidates(trainByDate.get(candidateDateKey) ?? []))
        .filter((row) => String(row?.dateKey ?? "").trim() !== dateKey)
    ...
    const excludedNegativeKeys = new Set(
      [
        ...
        ...matchedControlRows.map((row) => row?.rowKey),
      ].filter(Boolean),
    )
  ...
  return {
    ok,
    reason: ok ? null : "unsat_no_prejump_episode_transition_control_dataset",
    familyId,
    surfaceName: PERFECT_PROTOTYPE_PREJUMP_CONTEXTUAL_SURFACE,
    tokenizerOptions,
    ...
    summary: {
      ...
      executionFeasibleMatchedControlDateCount: new Set(
        uniqueMatchedControlNegativeRows.map((row) => row?.dateKey).filter(Boolean),
      ).size,
