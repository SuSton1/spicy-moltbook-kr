...
export const applyPerfectPrototypeSupportBoundaryResidualScorer = ({
  artifact,
  rows = [],
} = {}) =>
  (Array.isArray(rows) ? rows : []).map((row) => {
    ...
    const supportScore = featureContributions.reduce((sum, entry) => sum + Number(entry.contribution ?? 0), 0)
    const featureMatchCount = featureContributions.length
    const hardNegativeMargin = num(row?.numericFeatureMap?.[artifact?.marginFeatureKey])
    const falsePositivePressure = num(row?.numericFeatureMap?.[artifact?.falsePositivePressureFeatureKey])
    const abstainScore =
      Number.isFinite(num(supportScore)) && Number.isFinite(falsePositivePressure)
        ? supportScore - falsePositivePressure
        : Number.isFinite(num(supportScore))
          ? supportScore
          : null
    const selected =
      gatePassed &&
      ...
      Number.isFinite(num(supportScore)) &&
      Number(supportScore) >= Number(artifact?.selectThreshold ?? 0) &&
      Number.isFinite(hardNegativeMargin) &&
      hardNegativeMargin >= Number(artifact?.marginThreshold ?? 0) &&
      Number.isFinite(num(abstainScore)) &&
      Number(abstainScore) >= Number(artifact?.abstainThreshold ?? 0)
    return {
      row,
      gatePassed,
      featureMatchCount,
      supportScore: Number.isFinite(num(supportScore)) ? supportScore : null,
      hardNegativeMargin,
      falsePositivePressure,
      abstainScore: Number.isFinite(num(abstainScore)) ? abstainScore : null,
      selected,
    }
  })

export const summarizePerfectPrototypeSupportBoundaryResidualSelections = ({
  evaluations = [],
  supportCaseIds = [],
  minWindowSupport = 2,
} = {}) => {
