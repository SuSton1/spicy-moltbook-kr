...
export const TECHNIQUE_CLUSTER_BANK_DISCOVERY_MANIFEST_KIND = "technique_cluster_bank_discovery_manifest_v1"
...
export const selectTechniqueClusterBankDiscoveryPlanEntries = ({
  planArtifact,
...
export const buildTechniqueClusterBankDiscoveryRollingContract = ({
  techniqueContract,
  planArtifact,
  clusterBankId = null,
  selectedEntry = null,
  ...
  assertTechniqueClauseContext(
    chosenEntry,
    `technique cluster-bank rolling entry ${toText(chosenEntry.clusterBankId) || "<unknown>"}`,
  )
  ...
  return {
    ...
    hardStops: [
      `Derived technique cluster-bank discovery rolling contract for clusterBankId=${toText(chosenEntry.clusterBankId)}`,
      `bankId=${concrete.bankId}`,
      `scopeId=${resolvedScopeId}`,
      `lookbackCandidateId=${concrete.lookbackCandidateId}`,
...
export const writeTechniqueClusterBankDiscoveryRollingContract = async ({
  techniqueContract,
  planArtifact,
  clusterBankId,
  outPath,
} = {}) => {
  const payload = buildTechniqueClusterBankDiscoveryRollingContract({
    techniqueContract,
    planArtifact,
    clusterBankId,
  })
  ...
  if (!resolvedOutPath) {
    throw new Error("outPath is required for writeTechniqueClusterBankDiscoveryRollingContract")
  }
