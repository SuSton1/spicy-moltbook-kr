...
export const TECHNIQUE_CLUSTER_TEMPLATE_SCREEN_PLAN_KIND = "technique_cluster_template_screen_plan_v1"
export const TECHNIQUE_CLUSTER_TEMPLATE_SCREEN_MANIFEST_KIND = "technique_cluster_template_screen_manifest_v1"
...
export const selectTechniqueClusterTemplateScreenPlanTemplates = ({
  planArtifact,
  selectedTemplateIds = [],
  maxTemplates = null,
} = {}) => {
...
export const buildTechniqueClusterTemplateRollingContract = ({
  techniqueContract,
  planArtifact,
  templateId = null,
  ...
  assertTechniqueClauseContext(
    chosenTemplate,
    `technique cluster-template rolling entry ${toText(chosenTemplate.candidateTemplateId) || "<unknown>"}`,
  )
  ...
  return {
    ...
    hardStops: [
      `Derived technique cluster template rolling contract for candidateTemplateId=${toText(chosenTemplate.candidateTemplateId)}`,
      `bankId=${concreteBank.bankId}`,
...
export const writeTechniqueClusterTemplateRollingContract = async ({
  techniqueContract,
  planArtifact,
  templateId,
  outPath,
} = {}) => {
  const payload = buildTechniqueClusterTemplateRollingContract({
    techniqueContract,
    planArtifact,
    templateId,
  })
  ...
  if (!resolvedOutPath) {
    throw new Error("outPath is required for writeTechniqueClusterTemplateRollingContract")
  }
