...
export const buildPerfectPrototypeDailyCanonicalWinnerLabels = ({
  family,
  ...
  for (const [dateKey, rows] of byDate.entries()) {
    ...
    const sortedPositive = positives
      .map((row) => ({ row, netRet: eventNetRet(row), score: winnerBaseScore(row) }))
      .sort(
        (left, right) =>
    ...
    if (canonicalPositiveDateSet.has(dateKey)) {
      ...
      const runnerUps = sortedNegative
  ...
  return {
    ...
    reason:
      missingPositiveWinnerDateCount > 0
        ? "unsat_missing_canonical_positive_winners"
        : ok
          ? null
          : "unsat_no_daily_canonical_winner_labels",
