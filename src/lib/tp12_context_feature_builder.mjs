...
const contextFeatureRow = ({ candidate, candle, dateContext, contextMode }) => ({
  ...
  d0ClosePressurePct: candle.openToCloseReturn,
  d0TradingValue: candle.tradedValue,
  d0TradingValueRel20: candle.tradedValueRel20,
  d0CloseLocation: candle.closeLocation,
  d0RangePct: candle.rangePct,
  ...
  try {
    await iterateJsonlMaybeGzip(candidatePath, {
      ...
      await writeJsonlRow(writer.stream, {
        ...
        d0TradingValue: candle.tradedValue,
        d0TradingValueRel20: candle.tradedValueRel20,
        d0TradingValueRankPct: rank.tradedValueRankPct,
        d0CloseLocation: candle.closeLocation,
        d0RangePct: candle.rangePct,
        openToCloseReturn: candle.openToCloseReturn,
        return1d: candle.return1d,
