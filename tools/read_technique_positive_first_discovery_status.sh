...
usage() {
  cat <<'USAGE'
Usage: bash tools/read_technique_positive_first_discovery_status.sh --run-id=<run_id> [--tail-lines=N]
Server-only reader for detached technique positive-first discovery status.
USAGE
}
...
console.log(JSON.stringify({
  kind: "technique_positive_first_detached_status_v1",
