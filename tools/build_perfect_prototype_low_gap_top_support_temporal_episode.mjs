...
import { calibratePerfectPrototypeSupportTemporalAnchorScorer } from "../src/lib/perfect_prototype_support_temporal_anchor_calibrate.mjs"
import { buildPerfectPrototypeSupportTemporalAnchorUnsat } from "../src/lib/perfect_prototype_support_temporal_anchor_unsat.mjs"

const usage = () => {
  console.error(`Usage:
  node tools/build_perfect_prototype_low_gap_top_support_temporal_episode.mjs \\
...
const writeUnsat = async ({ outDir, value }) =>
  writeJson(path.join(outDir, "no_support_temporal_episode_summary.json"), value)
...
const main = async () => {
  ...
  const scorer = calibratePerfectPrototypeSupportTemporalAnchorScorer({
    family,
  ...
  await writeJson(path.join(args.outDir, "support_temporal_anchor_scorer_summary.json"), scorer)
  if (!scorer.ok) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportTemporalAnchorUnsat(scorer),
    })
    return
  }

  await writeJson(path.join(args.outDir, "support_temporal_anchor_artifact.json"), scorer.artifact)
  await writeJson(path.join(args.outDir, "support_temporal_anchor_train_summary.json"), scorer.trainSummary)
  await writeJson(path.join(args.outDir, "support_temporal_anchor_oos_summary.json"), scorer.oosSummary)
  await writeJson(path.join(args.outDir, "support_temporal_anchor_solution_summary.json"), {
    ok: true,
    supportFitExcluded: scorer.supportFitExcluded === true,
    supportLeaveOneOutRecovered: scorer.supportLeaveOneOutRecovered === true,
    episodeFeatureCount: family.summary?.episodeFeatureCount ?? 0,
    roleTopologyFeatureCount: family.summary?.roleTopologyFeatureCount ?? 0,
    roleTopologySignatureCount: family.summary?.roleTopologySignatureCount ?? 0,
    selectedFeaturePoolCount: scorer.selectedFeaturePoolCount ?? 0,
    candidateCount: scorer.candidateCount ?? 0,
    qualifiedCandidateCount: scorer.qualifiedCandidateCount ?? 0,
    supportMatched: scorer.supportMatched ?? [],
    trainSummary: scorer.trainSummary,
    oosSummary: scorer.oosSummary,
    artifactHashBeforeAcceptance: scorer.artifactHashBeforeAcceptance ?? null,
    artifactHashAfterAcceptance: scorer.artifactHashAfterAcceptance ?? null,
    artifactPath: path.join(args.outDir, "support_temporal_anchor_artifact.json"),
  })
}
