...
usage() {
  cat <<'EOF'
Usage: bash tools/run_stepb_1d_tp12_no_gap_addon_window.sh --contract-path=PATH --addon-contract-path=PATH --window-id=<id> --source-run-id=<source_run_id> --run-id=<run_id> [--candle-path=PATH] [--max-search-states=N] [--max-rules=N]
Server-only helper that reruns exact TP12 no-stop mining on one no-gap addon candidate under a fixed rolling contract.
EOF
}
...
[[ -n "$ADDON_CONTRACT_PATH" ]] || fatal "--addon-contract-path is required"
...
const emitNumber = (key, value) => console.log(`${key}=${Number(value)}`)
emit("RESOLVED_CONTRACT_PATH", contract.contractPath)
emit("CANDIDATE_ID", contract.scopeId)
...
SELECTION_LINE_ID="stepb_dplus1_plus_lite_target12_no_gap_${candidate_slug}_probe"
...
write_no_rules_artifacts() {
  ...
  local reason="$3"
  node --input-type=module - "$summary_path" "$freeze_result_path" "$reason" "$WINDOW_ID" "$WINDOW_KIND" "$CANDIDATE_ID" <<'NODE'
...
node tools/build_tp12_no_gap_addon_pack.mjs \
  --contract-path="$ADDON_CONTRACT_PATH" \
...
if [[ "$TRAIN_MATCHED_ROWS" -lt 1 || "$OOS_MATCHED_ROWS" -lt 1 ]]; then
  write_no_rules_artifacts "$NO_RULES_SUMMARY_PATH" "$FREEZE_RESULT_PATH" "unsat_no_scope_rows"
else
...
NODE
  ...
  if [[ "$TRAIN_RULE_COUNT" -lt 1 ]]; then
    ...
    node tools/build_curated_perfect_prototype_catalog.mjs \
      ...
      --note="line=${SELECTION_LINE_ID} candidateId=${CANDIDATE_ID} windowId=${WINDOW_ID} windowKind=${WINDOW_KIND} sourceRunId=${SOURCE_RUN_ID} surface=${EXPECTED_CONTEXT_SURFACE} splitPolicy=${SPLIT_POLICY} foldScheme=${FOLD_SCHEME} train=${TRAIN_START}:${TRAIN_END} oos=${OOS_START}:${OOS_END}" \
      > "$FREEZE_RESULT_PATH"
...
node --input-type=module - "$RUNTIME_SUMMARY_PATH" "$WINDOW_ID" "$WINDOW_KIND" "$CANDIDATE_ID" "$SOURCE_RUN_ID" "$RUN_ID" "$TRAIN_START" "$TRAIN_END" "$OOS_START" "$OOS_END" "$MAX_SEARCH_STATES_EFFECTIVE" "$MAX_RULES" "$elapsed_sec" "$CONFIG_PATH" "$FREEZE_RESULT_PATH" "$NO_RULES_SUMMARY_PATH" <<'NODE'
...
echo "[done] tp12 no-gap addon window completed candidateId=${CANDIDATE_ID} runId=${RUN_ID} windowId=${WINDOW_ID} sourceRunId=${SOURCE_RUN_ID}"
