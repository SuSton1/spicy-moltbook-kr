...
DEFAULT_CONFIG_PATH="${PERFECT_PROTO_V60B_CONFIG_PATH:-config/lab.config.server.lite.stepb_dplus1_plus_lite.json}"
DEFAULT_REGISTRY_PATH="config/ops/live_priority_registry.server.json"
DEFAULT_RUN_ID="perfect_proto_live_line_failure_veto_overlay_report_v60b_$(date +%Y%m%d_%H%M%S)"
DEFAULT_INCLUDE_LINE_IDS="1d_primary,7d_primary,8d_primary,1d_secondary,7d_secondary,8d_secondary"
...
if [[ "$ROOT_REAL" != "$EXPECTED_REAL" ]]; then
  echo "[fatal] server_run_live_line_failure_veto_overlay_report.sh must run from $EXPECTED_REAL, got $ROOT_REAL" >&2
  exit 4
fi
...
REPORT_DIR="$RUN_DIR/step-perfect-prototype-live-line-failure-veto-overlay-report"
...
NODE
  ...
  apply_strict_label_boundary_to_daily_pack "$TRAIN_PACK_DIR" "$TRAIN_START" "$TRAIN_END" "decision_date_only"
  apply_strict_label_boundary_to_daily_pack "$OOS_PACK_DIR" "$OOS_START" "$OOS_END" "decision_date_only"
done < <(
...
const selectedLines = includeLineIds.map((lineId) => {
  ...
  if (String(line?.runnerType ?? "").trim() !== "plus_lite_same_day_recent") {
    throw new Error(`lineId=${lineId} has unsupported runnerType=${line?.runnerType ?? "null"} for v60b wrapper`)
  }
  return line
})
const seen = new Set()
...
node "$ROOT_DIR/tools/build_perfect_prototype_live_line_failure_veto_overlay_report.mjs" \
  --registry="$REGISTRY_PATH" \
  --scope-manifest="$SCOPE_MANIFEST_PATH" \
  --out-dir="$REPORT_DIR" \
  --candle-path="$CANDLE_PATH"

echo "[ok] v60b live-line failure/veto overlay diagnostic complete"
echo "runId=$RUN_ID"
