...
const negatives = [
  makeRow({ sourceId: "n1", symbol: "N1", dateKey: "2026-01-05", outcomeHitTarget: false, foldId: 1, windowId: 1, posDistance: 0.32, margin: 0.18, compatibility: 2.1 }),
  makeRow({ sourceId: "n2", symbol: "N2", dateKey: "2026-01-16", outcomeHitTarget: false, foldId: 1, windowId: 1, posDistance: 0.34, margin: 0.12, compatibility: 1.9 }),
  makeRow({ sourceId: "n3", symbol: "N3", dateKey: "2026-02-09", outcomeHitTarget: false, foldId: 2, windowId: 2, posDistance: 0.36, margin: 0.10, compatibility: 1.8 }),
  makeRow({ sourceId: "n4", symbol: "N4", dateKey: "2026-02-20", outcomeHitTarget: false, foldId: 2, windowId: 2, posDistance: 0.37, margin: 0.08, compatibility: 1.7 }),
  makeRow({ sourceId: "n5", symbol: "N5", dateKey: "2026-03-08", outcomeHitTarget: false, foldId: 3, windowId: 3, posDistance: 0.39, margin: 0.05, compatibility: 1.6 }),
  makeRow({ sourceId: "n6", symbol: "N6", dateKey: "2026-03-20", outcomeHitTarget: false, foldId: 3, windowId: 3, posDistance: 0.40, margin: 0.04, compatibility: 1.5 }),
  makeRow({ sourceId: "n7", symbol: "N7", dateKey: "2026-04-06", outcomeHitTarget: false, foldId: 4, windowId: 4, posDistance: 0.42, margin: 0.03, compatibility: 1.4 }),
  makeRow({ sourceId: "n8", symbol: "N8", dateKey: "2026-04-18", outcomeHitTarget: false, foldId: 4, windowId: 4, posDistance: 0.43, margin: 0.02, compatibility: 1.3 }),
]
...
const family = buildPerfectPrototypeSupportBoundaryResidualFamily({
  cohort: bridge,
})

assert.equal(bridge.ok, true)
assert.equal(family.ok, true)
assert.equal(family.supportFitExcluded, true)
assert.ok(Number(family.summary?.boundaryResidualFeatureCount ?? 0) >= 8)
assert.ok(Number(family.summary?.boundaryResidualPositiveGroupCount ?? 0) >= 1)
assert.ok(
  Number(
    family.supportCaseViews?.[0]?.numericFeatureMap?.["sig.boundary.supportRecoveryPotential"] ??
      Number.NEGATIVE_INFINITY,
  ) > 0,
)

console.log("ok: support boundary residual family")
