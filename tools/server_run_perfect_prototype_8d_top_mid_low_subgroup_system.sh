...
DEFAULT_RUN_ID="perfect_proto_8d_top_mid_low_donor_subgroup_system_v61_$(date +%Y%m%d_%H%M%S)"
DEFAULT_INCLUDE_LINE_IDS="8d_primary,8d_secondary"
...
if [[ "$ROOT_REAL" != "$EXPECTED_REAL" ]]; then
  echo "[fatal] server_run_perfect_prototype_8d_top_mid_low_subgroup_system.sh must run from $EXPECTED_REAL, got $ROOT_REAL" >&2
  exit 4
fi
...
MIN_MATCHED_DATES="10"
MIN_MATCHED_MONTHS="6"
MIN_MATCHED_FOLDS="4"
MIN_SELECTION_FREQUENCY="0.6"
MIN_FOLD_PRESENCE_COUNT="3"
MIN_WINDOW_PRESENCE_COUNT="3"
...
for arg in "$@"; do
  case "$arg" in
    ...
    --min-matched-dates=*) MIN_MATCHED_DATES="${arg#*=}" ;;
    --min-matched-months=*) MIN_MATCHED_MONTHS="${arg#*=}" ;;
    --min-matched-folds=*) MIN_MATCHED_FOLDS="${arg#*=}" ;;
    --min-selection-frequency=*) MIN_SELECTION_FREQUENCY="${arg#*=}" ;;
    --min-fold-presence-count=*) MIN_FOLD_PRESENCE_COUNT="${arg#*=}" ;;
    --min-window-presence-count=*) MIN_WINDOW_PRESENCE_COUNT="${arg#*=}" ;;
    --max-top1-date-hit-share=*) MAX_TOP1_DATE_HIT_SHARE="${arg#*=}" ;;
    --max-manifests=*) MAX_MANIFESTS="${arg#*=}" ;;
...
REPORT_DIR="$RUN_DIR/step-perfect-prototype-8d-top-mid-low-subgroup-system"
SCOPE_MANIFEST_PATH="$RUN_DIR/8d_subgroup_scope_manifest.json"
...
const selectedLines = includeLineIds.map((lineId) => {
  ...
  if (String(line?.runnerType ?? "").trim() !== "plus_lite_same_day_recent") {
    throw new Error(`lineId=${lineId} has unsupported runnerType=${line?.runnerType ?? "null"} for v61 subgroup wrapper`)
  }
  return line
})
const seen = new Set()
...
node "$ROOT_DIR/tools/build_perfect_prototype_8d_top_mid_low_subgroup_system.mjs" \
  --scope-manifest="$SCOPE_MANIFEST_PATH" \
  --out-dir="$REPORT_DIR" \
  --candle-path="$CANDLE_PATH" \
  --min-matched-dates="$MIN_MATCHED_DATES" \
  --min-matched-months="$MIN_MATCHED_MONTHS" \
  --min-matched-folds="$MIN_MATCHED_FOLDS" \
  --min-selection-frequency="$MIN_SELECTION_FREQUENCY" \
  --min-fold-presence-count="$MIN_FOLD_PRESENCE_COUNT" \
  --min-window-presence-count="$MIN_WINDOW_PRESENCE_COUNT" \
  --max-top1-date-hit-share="$MAX_TOP1_DATE_HIT_SHARE" \
  --max-manifests="$MAX_MANIFESTS"

echo "[ok] v61 8d donor-aware subgroup system complete"
echo "runId=$RUN_ID"
