...
usage() {
  cat <<'EOF2'
Usage: bash tools/run_technique_template_rolling.sh --run-id=<run_id> --plan-path=PATH [--contract-path=PATH] [--template-ids=id1,id2] [--max-templates=N] [--window-group=screen|all] [--candle-path=PATH] [--max-rules=N] [--keep-source-runs]
Server-only helper that drives shortlist-selected technique templates into the fixed TP12 no-stop rolling wrapper.
EOF2
}
...
OUT_DIR="$ROOT_DIR/artifacts/runs/$RUN_ID/step-perfect-prototype-technique-template-screen"
...
if [[ -e "$OUT_DIR" ]]; then
  fatal "technique template screen out dir already exists: $OUT_DIR"
fi
...
const planArtifact = await loadTechniqueTemplateScreenPlan({ planPath, cwd: process.cwd() })
const templates = selectTechniqueTemplateScreenPlanTemplates({
  planArtifact,
  selectedTemplateIds: requestedTemplateIds,
  maxTemplates: numericMaxTemplates,
})
if (templates.length < 1) throw new Error("No technique templates selected")
await fs.writeFile(
  templatesTsvPath,
...
emit("SOURCE_BANK_ID", planArtifact.sourceBankId)
...
while IFS=$'\t' read -r TEMPLATE_ID BANK_ID MECHANISM_ID SCOPE_ID LOOKBACK_CANDIDATE_ID || [[ -n "${TEMPLATE_ID:-}" ]]; do
  ...
  node tools/build_technique_template_screen_candidate_contract.mjs \
  ...
  entry_json="$(
    node --input-type=module - "$TEMPLATE_ID" "$BANK_ID" "$MECHANISM_ID" "$SCOPE_ID" "$LOOKBACK_CANDIDATE_ID" "$CHILD_RUN_ID" "$CHILD_CONTRACT_PATH" "$CHILD_SUMMARY_PATH" "$CHILD_REPORT_PATH" <<'NODE'
const [
  candidateTemplateId,
  bankId,
  mechanismId,
...
node --input-type=module - "$MANIFEST_PATH" "$RUN_ID" "$PLAN_PATH" "$PLAN_CONTRACT_ID" "$SOURCE_BANK_ID" "$WINDOW_GROUP" "$MANIFEST_TEMPLATES_JSONL" "$TEMPLATE_IDS" "$MAX_TEMPLATES" <<'NODE'
import fs from "node:fs/promises"

import { writeJson } from "./src/lib/io.mjs"
import { TECHNIQUE_TEMPLATE_SCREEN_MANIFEST_KIND } from "./src/lib/technique_template_screen_contract.mjs"

const [manifestPath, runId, planPath, planContractId, sourceBankId, windowGroup, templatesJsonlPath, requestedTemplateIdsCsv, maxTemplates] = process.argv.slice(2)
const lines = String(await fs.readFile(templatesJsonlPath, "utf8")).split("\n").map((line) => line.trim()).filter(Boolean)
const templates = lines.map((line, index) => {
  try {
    return JSON.parse(line)
  } catch (error) {
    throw new Error(`Malformed technique template screen manifest entry at ${templatesJsonlPath}:${index + 1}: ${error instanceof Error ? error.message : String(error)}`)
  }
})
await writeJson(manifestPath, {
  kind: TECHNIQUE_TEMPLATE_SCREEN_MANIFEST_KIND,
...
echo "[done] technique template rolling completed runId=${RUN_ID} templates=${SELECTED_TEMPLATE_COUNT} summary=${SUMMARY_PATH}"
