...
usage() {
  cat <<'USAGE'
Usage: bash tools/launch_technique_episode_matched_control_purity_slice_detached.sh --run-id=<run_id> [forwarded args...]
Server-only detached launcher for technique episode matched-control purity slice discovery.
USAGE
}
...
if [[ -f "$LAUNCH_PID" ]]; then
  ...
  if [[ -n "$existing_pid" ]] && kill -0 "$existing_pid" 2>/dev/null; then
    fatal "detached purity-slice run already running for run_id=$RUN_ID pid=$existing_pid"
  fi
fi

printf -v run_cmd '%q ' bash tools/run_technique_episode_matched_control_purity_slice.sh "${FORWARDED_ARGS[@]}"
...
if ! kill -0 "$child_pid" 2>/dev/null; then
  ...
  fatal "detached purity-slice run failed to stay alive after launch"
fi
...
await writeJson(launchStatusPath, {
  kind: "technique_episode_matched_control_purity_slice_detached_launch_v1",
