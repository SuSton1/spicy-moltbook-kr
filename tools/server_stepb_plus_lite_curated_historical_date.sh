...
DEFAULT_DATE="$(date +%F)"
DEFAULT_RUN_ID_PREFIX="${STEPB_PLUS_LITE_HISTORICAL_WRAPPER_RUN_ID_PREFIX:-perfect_proto_stepb_plus_lite_curated_historical_date}"
...
usage() {
  cat <<EOF
Usage: bash tools/server_stepb_plus_lite_curated_historical_date.sh \
  ...
  [--config=/abs/path/config.json] [--run-id=<run_id>] [--allow-mutable-catalog=true]
EOF
}
...
if [[ "$ROOT_REAL" != "$EXPECTED_REAL" ]]; then
  fatal "server-only historical-date wrapper must run from $EXPECTED_REAL, got $ROOT_REAL"
fi
...
if [[ -z "$CATALOG_PATH" ]]; then
  fatal "missing --catalog. historical-date apply requires an explicit frozen catalog path."
fi
...
if [[ "$ALLOW_MUTABLE_CATALOG" != "true" && "$CATALOG_PATH" != *"/artifacts/curated/frozen/"*"/catalog.json" ]]; then
  fatal "historical-date apply requires an immutable frozen catalog path under artifacts/curated/frozen/"
fi
...
if (expectedDiscoveryKind === "same_day_plus_recent" && contract.isSameDayPlusRecentUniverse !== true) {
  throw new Error(
    `server_stepb_plus_lite_curated_historical_date.sh requires same_day_plus_recent discovery universe, got ${discoveryUniverseId}`,
  )
}
if (expectedDiscoveryKind === "recent_impulse" && contract.isRecentImpulseUniverse !== true) {
  throw new Error(
    `server_stepb_plus_lite_curated_historical_date.sh requires recent_impulse discovery universe, got ${discoveryUniverseId}`,
  )
}
...
if (hasParentRule || !allPrototypeChildRules) {
  throw new Error("historical-date wrapper only supports Step-B child PP_* catalogs.")
}
...
if (surface !== String(expectedSurface).trim().toLowerCase()) {
  throw new Error(`historical-date wrapper requires surface=${expectedSurface}, got ${surface || "null"}`)
}
...
if (!datasetContract || typeof datasetContract !== "object") {
  throw new Error(`frozen catalog is missing datasetContract: ${manifestPath}`)
}
...
eval "$(
  node --input-type=module - "$ROOT_DIR/data/candle_daily.jsonl" "$ROOT_DIR/data/universe_daily.jsonl" "$TARGET_DATE" <<'NODE'
import fs from "node:fs"
import readline from "node:readline"

const [candlePath, universePath, targetDate] = process.argv.slice(2)

const readDateStats = async (filePath) => {
  let latest = ""
  ...
  try {
    for await (const line of rl) {
      ...
      if (dateKey === targetDate) foundTarget = true
    }
  } finally {
    rl.close()
  }
  return { latest, foundTarget }
}

const candleStats = await readDateStats(candlePath)
const universeStats = await readDateStats(universePath)
console.log(`CANDLE_LATEST_DATE=${candleStats.latest}`)
console.log(`UNIVERSE_LATEST_DATE=${universeStats.latest}`)
console.log(`CANDLE_HAS_TARGET_DATE=${candleStats.foundTarget ? "1" : "0"}`)
console.log(`UNIVERSE_HAS_TARGET_DATE=${universeStats.foundTarget ? "1" : "0"}`)
NODE
)"
...
if [[ "$TARGET_DATE" > "$CANDLE_LATEST_DATE" ]]; then
  fatal "requested --date must not exceed latest common data date: date=$TARGET_DATE latest=$CANDLE_LATEST_DATE"
fi
if [[ "${CANDLE_HAS_TARGET_DATE:-0}" != "1" || "${UNIVERSE_HAS_TARGET_DATE:-0}" != "1" ]]; then
  fatal "requested --date is not present in both candle/universe data: date=$TARGET_DATE"
fi
...
echo "[ok] step-b plus-lite curated historical-date run complete"
echo "runId=$RUN_ID"
