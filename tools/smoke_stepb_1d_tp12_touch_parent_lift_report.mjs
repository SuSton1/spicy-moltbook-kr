  ...
  applyPerfectPrototypeCatalogFreezeMetadata({
    catalog: {
      ...
      metadata: {
        ...
        selectionLineId: "stepb_dplus1_plus_lite_target12_low_gap_top_parent_lift_probe",
      },
      rules: [
        {
          ruleId: "RULE_A",
          familyId: "low_gap_top_continuation",
          tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:a"],
          precision: 1,
        },
        {
          ruleId: "RULE_B",
          familyId: "low_gap_top_continuation",
          tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:b"],
          precision: 1,
        },
        {
          ruleId: "RULE_C",
          familyId: "low_gap_top_continuation",
          tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:c"],
          precision: 1,
        },
      ],
    },
    sourceRunId: "smoke_tp12_touch_parent_lift",
    selectionMode: "union_all",
    selectionLineId: "stepb_dplus1_plus_lite_target12_low_gap_top_parent_lift_probe",
...
const main = async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tp12-touch-parent-lift-"))
  ...
  const outDir = path.join(tempRoot, "artifacts", "runs", "parent", "step-perfect-prototype-1d-tp12-touch-parent-lift-report")
  await Promise.all([
  ...
  await writeJson(path.join(evalDir, "open_eval_manifest.json"), {
    lineId: "stepb_dplus1_plus_lite_target12_low_gap_top_parent_lift_probe",
    selectionMode: "union_all",
  })

  const trainRows = [
    buildPackRow({ rowKey: "t1", dateKey: "2020-11-27", symbol: "T001", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:a", "tag:extra:b"] }),
    buildPackRow({ rowKey: "t2", dateKey: "2021-02-01", symbol: "T002", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:a"] }),
    buildPackRow({ rowKey: "t3", dateKey: "2021-08-02", symbol: "T003", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:b"] }),
    buildPackRow({ rowKey: "t4", dateKey: "2021-12-03", symbol: "T004", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:b", "tag:extra:c"] }),
    buildPackRow({ rowKey: "t5", dateKey: "2022-04-11", symbol: "T005", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:c"] }),
    buildPackRow({ rowKey: "t6", dateKey: "2022-12-13", symbol: "T006", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:a", "tag:extra:c"] }),
    buildPackRow({ rowKey: "t7", dateKey: "2023-03-06", symbol: "T007", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base"] }),
    buildPackRow({ rowKey: "t8", dateKey: "2023-08-16", symbol: "T008", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base"] }),
  ...
  const oosRows = [
    buildPackRow({ rowKey: "o1", dateKey: "2025-01-02", symbol: "O001", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:a", "tag:extra:b"] }),
    buildPackRow({ rowKey: "o2", dateKey: "2025-04-02", symbol: "O002", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:b", "tag:extra:c"] }),
    buildPackRow({ rowKey: "o3", dateKey: "2025-08-04", symbol: "O003", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base", "tag:extra:a", "tag:extra:c"] }),
    buildPackRow({ rowKey: "o4", dateKey: "2025-09-15", symbol: "O004", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base"] }),
    buildPackRow({ rowKey: "o5", dateKey: "2025-11-17", symbol: "O005", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:setup:base"] }),
    buildPackRow({ rowKey: "o6", dateKey: "2025-12-09", symbol: "O006", tokens: ["tag:scope:gap_top", "tag:shape:reclaim", "tag:veto:bad"] }),
  ]
  ...
  const candleRows = [
    ...
    ...buildPositiveSeries({ symbol: "T007", decisionDateKey: "2023-03-06", entryDateKey: "2023-03-07", followDateKey: "2023-03-08", exitDateKey: "2023-03-09" }),
    ...buildPositiveSeries({ symbol: "T008", decisionDateKey: "2023-08-16", entryDateKey: "2023-08-17", followDateKey: "2023-08-18", exitDateKey: "2023-08-21" }),
    ...
    ...buildPositiveSeries({ symbol: "O004", decisionDateKey: "2025-09-15", entryDateKey: "2025-09-16", followDateKey: "2025-09-17", exitDateKey: "2025-09-18" }),
    ...buildPositiveSeries({ symbol: "O005", decisionDateKey: "2025-11-17", entryDateKey: "2025-11-18", followDateKey: "2025-11-19", exitDateKey: "2025-11-20" }),
    ...buildNegativeSeries({ symbol: "O006", decisionDateKey: "2025-12-09", entryDateKey: "2025-12-10", followDateKey: "2025-12-11", exitDateKey: "2025-12-12" }),
  ]
  await writeJsonl(path.join(dataDir, "candle_daily.jsonl"), candleRows)

  await runNode(
    [
      path.join(process.cwd(), "tools", "build_stepb_1d_tp12_touch_parent_lift_report.mjs"),
      `--scope-run-id=${runId}`,
      ...
      `--max-seed-clusters=2`,
      `--max-parent-terms=8`,
      `--min-parent-train-selected-rows=4`,
      `--min-parent-train-dates=4`,
      `--min-parent-train-months=2`,
      `--min-parent-train-folds=2`,
      `--min-parent-train-precision=0.6`,
  ...
  const summary = await readJson(path.join(outDir, "touch_parent_lift_summary.json"), null)
  const verdict = await readJson(path.join(outDir, "touch_parent_lift_verdict.json"), null)
  assert.equal(summary?.parentLiftBank?.candidateCount >= 1, true)
  assert.equal(summary?.parentLiftBank?.qualifiedTermCount >= 1, true)
  assert.equal(Number.isFinite(Number(summary?.seedBundle?.termCount ?? Number.NaN)), true)
  assert.equal(Number.isFinite(Number(summary?.parentLiftBundle?.positiveBundle?.termCount ?? Number.NaN)), true)
  assert.equal(summary?.parentLiftBundle?.finalBundle?.oosSummary?.selectedRowCount >= 3, true)
  assert.equal(typeof verdict?.code, "string")
  console.log("ok: smoke_stepb_1d_tp12_touch_parent_lift_report")
}
