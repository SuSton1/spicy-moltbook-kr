...
DEFAULT_RULE_IDS_FILE="$ROOT_DIR/artifacts/curated/perfect_proto_live_rule_ids.txt"
DEFAULT_LABEL="live_curated_manual"
...
usage() {
  cat <<EOF
Usage: bash tools/server_rebuild_curated_perfect_prototype_catalog.sh --source-catalog=/abs/path/catalog.json [--rule-ids-file=/abs/path/ids.txt] [--out-path=/abs/path/catalog.json] [--label=<label>] [--note=<note>] [--allow-mutable-output=true]
EOF
}
...
echo "[ok] curated catalog rebuilt"
