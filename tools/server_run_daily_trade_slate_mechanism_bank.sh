...
DEFAULT_RUN_ID="perfect_proto_daily_trade_slate_mechanism_bank_v55_$(date +%Y%m%d_%H%M%S)"
...
FAMILY_DIR="$RUN_DIR/step-perfect-prototype-daily-trade-slate-mechanism-bank"
...
node "$ROOT_DIR/tools/build_perfect_prototype_daily_trade_slate_mechanism_bank.mjs" \
...
if [[ -f "$FAMILY_DIR/no_support_daily_trade_slate_mechanism_bank_summary.json" ]]; then
  echo "[fatal] no feasible daily trade-slate mechanism-bank artifact. see $FAMILY_DIR/no_support_daily_trade_slate_mechanism_bank_summary.json" >&2
  exit 42
fi

echo "[ok] daily trade-slate mechanism-bank solve complete"
echo "runId=$RUN_ID"
