...
usage() {
  cat <<'EOF'
Usage: bash tools/run_stepb_1d_tp12_no_stop_low_gap_top_rolling.sh --run-id=<run_id> [--contract-path=PATH] [--window-group=screen|final|all] [--window-ids=w1,w2] [--candle-path=PATH] [--max-rules=N]
Server-only orchestrator for the TP12 no-stop rolling LOW_GAP_TOP branch.
EOF
}
...
for window_id in "${window_ids_array[@]}"; do
  ...
  bash tools/run_stepb_1d_tp12_no_stop_low_gap_top_window.sh "${window_args[@]}"
...
node --input-type=module - "$MANIFEST_PATH" "$RUN_ID" "$WINDOW_GROUP" "$RESOLVED_CONTRACT_PATH" "$MANIFEST_WINDOWS_JSONL" "$WINDOW_ID_LIST" "$MAX_RULES" <<'NODE'
import fs from "node:fs/promises"

import { writeJson } from "./src/lib/io.mjs"

const [manifestPath, runId, windowGroup, contractPath, windowsJsonlPath, requestedWindowIdsRaw, maxRulesOverrideRaw] = process.argv.slice(2)
...
echo "[done] tp12 no-stop rolling completed runId=${RUN_ID} summary=${SUMMARY_PATH}"
