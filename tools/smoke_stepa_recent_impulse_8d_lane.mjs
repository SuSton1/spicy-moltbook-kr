...
const buildConfig = ({ dataDir, engine }) => ({
  ...
  periods: {
    ...
    discovery: { from: makeDate(5), to: makeDate(13) },
    online: { from: makeDate(14), to: makeDate(14) },
    lockbox: { from: makeDate(14), to: makeDate(14) },
  },
})
...
const main = async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stepa-recent-impulse-8d-"))
  ...
  await writeJsonl(path.join(dataDir, "symbol_master.jsonl"), [
    { symbol: "111111", name: "EightDayFollowThrough", type: "COMMON" },
  ])
  ...
  const expectedRows = [
    ...
    { symbol: "111111", dateKey: makeDate(13), stepALaneId: "recent_impulse_8d", impulseSourceDateKey: makeDate(5), impulseLookbackDays: 8 },
  ]
  ...
  for (const engine of engines) {
    ...
    if (Number(summary?.rawEventCandidates ?? 0) !== 9 || Number(summary?.passedEvents ?? 0) !== 9) {
      throw new Error(`${engine} summary raw/passed mismatch`)
    }
    for (let lookback = 1; lookback <= 8; lookback += 1) {
    ...
    if (Number(summary?.candidateRecentImpulseDedupedCount ?? 0) !== 8 || Number(summary?.passedRecentImpulseDedupedCount ?? 0) !== 8) {
      throw new Error(`${engine} deduped recent impulse summary mismatch`)
    }
    if (summary?.recentImpulseDiscovery?.enabled !== true || Number(summary?.recentImpulseDiscovery?.lookbackTradingDays) !== 8) {
