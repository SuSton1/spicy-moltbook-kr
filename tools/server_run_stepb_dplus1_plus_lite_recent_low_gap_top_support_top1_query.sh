...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_LOW_GAP_TOP_TOP1_QUERY_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_low_gap_top_counterfactual_outranking_residual_v45}_$(date +%Y%m%d_%H%M%S)"
...
TOP1_DIR="$RUN_DIR/step-perfect-prototype-support-top1-query"

node "$ROOT_DIR/tools/build_perfect_prototype_low_gap_top_support_top1_query.mjs" \
...
if [[ -f "$TOP1_DIR/no_support_top1_query_summary.json" ]]; then
  echo "[fatal] no feasible low-gap-top support top1 query ranker. see $TOP1_DIR/no_support_top1_query_summary.json" >&2
  exit 42
fi

echo "[ok] low-gap-top support top1 query solve complete"
echo "runId=$RUN_ID"
echo "top1Dir=$TOP1_DIR"
