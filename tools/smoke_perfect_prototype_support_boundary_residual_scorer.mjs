...
const oosRows = [
  makeRow({ sourceId: "o1", symbol: "O1", dateKey: "2026-05-02", outcomeHitTarget: true, foldId: 0, windowId: 0, posDistance: 0.12, margin: 1.27, compatibility: 4.2 }),
  makeRow({ sourceId: "o2", symbol: "O2", dateKey: "2026-05-07", outcomeHitTarget: true, foldId: 0, windowId: 0, posDistance: 0.16, margin: 1.19, compatibility: 3.9 }),
  makeRow({ sourceId: "o3", symbol: "O3", dateKey: "2026-05-11", outcomeHitTarget: true, foldId: 0, windowId: 0, posDistance: 0.19, margin: 1.14, compatibility: 3.6 }),
  makeRow({ sourceId: "o4", symbol: "X1", dateKey: "2026-05-05", outcomeHitTarget: false, foldId: 0, windowId: 0, posDistance: 0.41, margin: 0.05, compatibility: 1.5 }),
  makeRow({ sourceId: "o5", symbol: "X2", dateKey: "2026-05-09", outcomeHitTarget: false, foldId: 0, windowId: 0, posDistance: 0.46, margin: 0.01, compatibility: 1.1 }),
]
...
const family = buildPerfectPrototypeSupportBoundaryResidualFamily({ cohort: bridge })
...
assert.ok(Number(solution.trainSummary.trainMatchedDateCount ?? 0) >= 4)
assert.equal(solution.oosSummary.precision, 1)
assert.ok(Number(solution.oosSummary.selectedRowCount ?? 0) >= 3)
assert.ok(
  Array.isArray(solution.artifact?.supportCaseIds) &&
    solution.artifact.supportCaseIds.includes(PERFECT_PROTOTYPE_HAESUNG_SUPPORT_CASE_ID),
)

console.log("ok: support boundary residual scorer")
