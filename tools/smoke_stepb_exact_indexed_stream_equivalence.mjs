...
import { writeJsonAtomic, writeJsonl } from "../src/lib/io.mjs"
...
const buildFixtureRows = () => {
  ...
  return specs.map((spec, index) => ({
    ...
    sourceId: `indexed_stream_fixture_${String(index + 1).padStart(2, "0")}`,
    symbol: spec.symbol,
...
const main = async () => {
  const rows = enrichPerfectPrototypeRowsWithContext(buildFixtureRows(), {
    referenceRows: buildFixtureRows(),
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "stepb-exact-indexed-stream-"))
  try {
    const directIndexDir = path.join(tempRoot, "direct_index")
    const streamedIndexDir = path.join(tempRoot, "streamed_index")
    const mineOutDir = path.join(tempRoot, "mine_out")
    const fixtureInputPath = path.join(tempRoot, "fixture_templates_lite.jsonl")
    const tokenizerSpecPath = path.join(tempRoot, "tokenizer_spec.json")
    await writeJsonl(fixtureInputPath, rows)

    const directIndex = await buildPerfectPrototypeStepbExactIndex({
      rows,
    ...
    await writeJsonAtomic(tokenizerSpecPath, directIndex.snapshot.tokenizerSpec)

    const streamedIndex = await buildPerfectPrototypeStepbExactIndexedStream({
      cwd: process.cwd(),
      inputPath: fixtureInputPath,
      inputSha256: "d".repeat(64),
      tokenizerSpec: directIndex.snapshot.tokenizerSpec,
      tokenizerSpecPath,
      outDir: streamedIndexDir,
      options,
    })
    const indexed = await minePerfectPrototypeIndexed({
      cwd: process.cwd(),
      indexDir: streamedIndexDir,
      outDir: mineOutDir,
      options,
    })
    const rowProjectedMineOutDir = path.join(tempRoot, "mine_out_row_projected")
    const indexedRowProjected = await minePerfectPrototypeIndexed({
      cwd: process.cwd(),
      indexDir: streamedIndexDir,
      outDir: rowProjectedMineOutDir,
      options: {
        ...options,
        rowProjectedCandidateThreshold: 4,
      },
    })

    const directRules = direct.rules.map(normalizeRule)
    const indexedRules = indexed.rules.map(normalizeRule)
    const directMatches = normalizeMatches(direct.matches)
    const indexedMatches = normalizeMatches(indexed.matches)
    const indexedRowProjectedMatches = normalizeMatches(indexedRowProjected.matches)
    const directCoverage = normalizeCoverage(direct.coverage)
    const indexedCoverage = normalizeCoverage(indexed.coverage)
    const indexedRowProjectedCoverage = normalizeCoverage(indexedRowProjected.coverage)
    const indexedRowProjectedRules = indexedRowProjected.rules.map(normalizeRule)

    assert.equal(streamedIndex.manifest.layout, PERFECT_PROTOTYPE_STEPB_EXACT_INDEX_LAYOUT_INDEXED_ONLY_V1)
    assert.equal(streamedIndex.manifest.searchMode, PERFECT_PROTOTYPE_EXACT_INDEXED_KERNEL_SEARCH_MODE)
    assert.equal(typeof streamedIndex.manifest.rowTokenOffsetsBinPath, "string")
    assert.equal(typeof streamedIndex.manifest.rowTokenIdsBinPath, "string")
    assert.ok(Number.isInteger(Number(streamedIndex.manifest.rowTokenIdWidthBits ?? NaN)))
    assert.deepEqual(indexedRules, directRules)
    assert.deepEqual(indexedRowProjectedRules, directRules)
    assert.deepEqual(indexedMatches, directMatches)
    assert.deepEqual(indexedRowProjectedMatches, directMatches)
    assert.deepEqual(indexedCoverage, directCoverage)
    assert.deepEqual(indexedRowProjectedCoverage, directCoverage)
    assert.equal(indexed.exploredStates, direct.exploredStates)
    assert.equal(indexedRowProjected.exploredStates, direct.exploredStates)
    assert.equal(sha256(indexedRules), sha256(directRules))
    assert.equal(sha256(indexedRowProjectedRules), sha256(directRules))
    assert.equal(sha256(indexedMatches), sha256(directMatches))
    assert.equal(sha256(indexedRowProjectedMatches), sha256(directMatches))
  } finally {
    await fs.rm(tempRoot, { recursive: true, force: true })
  }

  console.log("ok: Step-B exact indexed streamed builder equivalence")
}
