...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_SHADOW_WINNER_SLATE_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_shadow_winner_slate_top1_v50}_$(date +%Y%m%d_%H%M%S)"
...
SHADOW_DIR="$RUN_DIR/step-perfect-prototype-shadow-winner-slate"

node "$ROOT_DIR/tools/build_perfect_prototype_shadow_winner_slate.mjs" \
...
if [[ -f "$SHADOW_DIR/no_support_shadow_winner_slate_summary.json" ]]; then
  echo "[fatal] no feasible shadow winner-slate artifact. see $SHADOW_DIR/no_support_shadow_winner_slate_summary.json" >&2
  exit 42
fi

echo "[ok] shadow winner-slate solve complete"
echo "runId=$RUN_ID"
echo "shadowDir=$SHADOW_DIR"
