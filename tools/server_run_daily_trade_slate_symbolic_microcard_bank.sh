...
DEFAULT_RUN_ID="perfect_proto_daily_trade_slate_symbolic_microcard_bank_v56_$(date +%Y%m%d_%H%M%S)"
...
FAMILY_DIR="$RUN_DIR/step-perfect-prototype-daily-trade-slate-symbolic-microcard-bank"
...
node "$ROOT_DIR/tools/build_perfect_prototype_daily_trade_slate_symbolic_microcard_bank.mjs" \
  ...
  --microcard-min-date-count=4 \
  --microcard-max-count=5 \
  --microcard-max-share=0.8 \
  "${EXTRA_ARGS[@]}"

if [[ -f "$FAMILY_DIR/no_support_daily_trade_slate_symbolic_microcard_bank_summary.json" ]]; then
  echo "[fatal] no feasible daily trade-slate symbolic microcard artifact. see $FAMILY_DIR/no_support_daily_trade_slate_symbolic_microcard_bank_summary.json" >&2
  exit 42
fi

echo "[ok] daily trade-slate symbolic microcard solve complete"
echo "runId=$RUN_ID"
