...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_LOW_GAP_TOP_BOUNDARY_RESIDUAL_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_low_gap_top_boundary_residual_scorer_v37}_$(date +%Y%m%d_%H%M%S)"
...
SCORER_DIR="$RUN_DIR/step-perfect-prototype-boundary-residual-scorer"

node "$ROOT_DIR/tools/build_perfect_prototype_low_gap_top_boundary_residual_scorer.mjs" \
...
if [[ -f "$SCORER_DIR/no_support_boundary_residual_scorer_summary.json" ]]; then
  echo "[fatal] no feasible low-gap-top boundary residual scorer. see $SCORER_DIR/no_support_boundary_residual_scorer_summary.json" >&2
  exit 42
fi

echo "[ok] low-gap-top boundary residual scorer solve complete"
echo "runId=$RUN_ID"
echo "scorerDir=$SCORER_DIR"
