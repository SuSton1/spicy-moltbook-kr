...
import {
  buildTechniqueTemplateRollingContract,
...
const techniqueContract = {
  ...
  templateScreen: {
    ...
    minUsableWindows: 5,
    minRollingOosHitRate: 0.28,
    minOosYearsWithHitGe2: 5,
    minSignalsPer20TradingDays: 2,
    maxSignalsPer20TradingDays: 8,
    maxTop1DateShare: 0.1,
  },
  hardStops: ["do not treat bank aggregates as single-template results"],
...
const planArtifact = {
  kind: "technique_template_screen_plan_v1",
  ...
  selectedTemplates: [
    {
      ...
      observedHitCount: 10,
      observedHitRate: 0.333333,
      totalEventCount: 30,
      totalHitCount: 10,
      totalHitRate: 0.333333,
      coveredYears: 8,
      yearsWithHitGe2: 8,
      yearsWithHitGe1: 8,
...
const selectedTemplates = selectTechniqueTemplateScreenPlanTemplates({
  planArtifact,
  selectedTemplateIds: ["tpl_a"],
})
assert.equal(selectedTemplates.length, 1)

const rollingContract = buildTechniqueTemplateRollingContract({
  techniqueContract,
  planArtifact,
  templateId: "tpl_a",
})
...
assert.equal(rollingContract.techniqueTemplateScreen.bankId, "MA_RETEST__LOW_GAP_TOP__lb5")
assert.equal(rollingContract.techniqueTemplateScreen.candidateTemplateId, "tpl_a")
assert.deepEqual(rollingContract.techniqueTemplateScreen.anchorClauseIds, ["anchor_ma120_break"])
assert.deepEqual(rollingContract.techniqueTemplateScreen.confirmClauseIds, [
  "confirm_close_near_high",
  "confirm_sponsor_quality",
])
console.log("ok smoke_technique_template_screen_contract")
