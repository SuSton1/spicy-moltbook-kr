...
const main = async () => {
  ...
  const policy = assertPerfectPrototypeServerWorkspace({
    cwd,
    toolName: "build_tp12_no_gap_addon_pack",
  })
  const contractPath = String(getFlag(parsed.flags, "contract-path", "meta/tp12_no_gap_addon_research_contract.json")).trim()
  ...
  const candidateId = String(getFlag(parsed.flags, "candidate-id", "")).trim()
  ...
  if (!inputPath || !outDir || !candidateId) {
    throw new Error(
      "Usage: node tools/build_tp12_no_gap_addon_pack.mjs --input=<daily_pack.jsonl> --out-dir=<dir> --candidate-id=<id> [--contract-path=PATH] [--source-run-id=<run>] [--source-stage-label=train|oos] [--row-contract=open_eval_recent_impulse_1d] [--require-nonempty=true|false]",
    )
  }
  assertPerfectPrototypeServerPaths({
    entries: [
  ...
  const candidateSpec = resolveTp12NoGapAddonCandidate({
    addonContract,
    candidateId,
  })
  const summary = await filterTp12NoGapAddonPack({
    inputPath,
    outDir,
    candidateSpec,
    sourceRunId,
