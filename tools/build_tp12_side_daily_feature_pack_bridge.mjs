...
const main = async () => {
  ...
  const sideFeatureRaw = toText(getFlag(flags, "side-feature-path"))
  const gateId = toText(getFlag(flags, "gate-id"))
  const outRaw = toText(getFlag(flags, "out"))
  if (!featurePackRaw || !sideFeatureRaw || !gateId || !outRaw) {
    throw new Error(
      "build_tp12_side_daily_feature_pack_bridge requires --feature-pack-path --side-feature-path --gate-id --out",
    )
  }
  const cwd = process.cwd()
  const summary = await buildTp12SideDailyFeaturePackBridge({
    featurePackPath: path.resolve(cwd, featurePackRaw),
    sideFeaturePath: path.resolve(cwd, sideFeatureRaw),
    gateId,
