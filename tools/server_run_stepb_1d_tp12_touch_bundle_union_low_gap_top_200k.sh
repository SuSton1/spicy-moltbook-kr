...
DEFAULT_RUN_ID="perfect_proto_stepb_1d_tp12_low_gap_top_touch_bundle_union_200k_$(date +%Y%m%d_%H%M%S)"
...
MAX_POSITIVE_TERMS="8"
MAX_VETO_TOKENS="2"
MIN_POSITIVE_IMPROVEMENT="0.01"
MIN_VETO_IMPROVEMENT="0.005"
MIN_TRAIN_TERM_MONTHS="2"
MIN_TRAIN_TERM_FOLDS="3"
MIN_SELECTED_ROWS_AFTER_VETO="60"
MIN_SELECTED_DATES_AFTER_VETO="15"
...
MIN_VETO_NET_GAIN="1"
MIN_VERDICT_OOS_SELECTED_ROWS="100"
MIN_VERDICT_OOS_DATES="60"
...
usage() {
  cat <<EOF2
Usage: bash tools/server_run_stepb_1d_tp12_touch_bundle_union_low_gap_top_200k.sh [--run-id=<run_id>] [--scope-run-id=<child_run_id>] [--scope-label=LOW_GAP_TOP] [--candle-path=<data/candle_daily.jsonl>]
Runs the server-only LOW_GAP_TOP TP12 touch bundle-union report against an existing filtered child run.
Defaults:
...
OUT_DIR="$PARENT_RUN_DIR/step-perfect-prototype-1d-tp12-touch-bundle-union-report"
mkdir -p "$OUT_DIR"

started_at="$(date +%s)"
run_report_node tools/build_stepb_1d_tp12_touch_bundle_union_report.mjs \
...
cat <<EOF2
[done] 1D TP12 LOW_GAP_TOP touch bundle-union completed
  runId=$RUN_ID
  scopeRunId=$SCOPE_RUN_ID
  outDir=$OUT_DIR
