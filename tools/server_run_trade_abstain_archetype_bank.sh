...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_TRADE_ABSTAIN_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_CONFIG_PATH="${PERFECT_PROTO_TRADE_ABSTAIN_CONFIG_PATH:-config/lab.config.server.lite.stepb_dplus1_plus_lite_lane_local.json}"
DEFAULT_RUN_ID="perfect_proto_trade_abstain_archetype_winner_bank_v54_$(date +%Y%m%d_%H%M%S)"
...
FAMILY_DIR="$RUN_DIR/step-perfect-prototype-trade-abstain-archetype-bank"
...
node "$ROOT_DIR/tools/extract_perfect_prototype_positive_dates.mjs" \
  --input="$TRAIN_INPUT_PATH" \
  --out="$TRAIN_POSITIVE_DATES_PATH"
node "$ROOT_DIR/tools/extract_perfect_prototype_positive_dates.mjs" \
  --input="$OOS_INPUT_PATH" \
  --out="$OOS_POSITIVE_DATES_PATH"
...
node "$ROOT_DIR/tools/build_perfect_prototype_trade_abstain_archetype_bank.mjs" \
  ...
  --control-pool-size=5 \
  --same-date-negative-pool-size=2 \
  --gate-max-feature-pool=5 \
  --gate-max-feature-count=3 \
  --archetype-min-dates=2 \
  --archetype-max-count=4 \
  "${EXTRA_ARGS[@]}"

if [[ -f "$FAMILY_DIR/no_support_trade_abstain_archetype_winner_bank_summary.json" ]]; then
  echo "[fatal] no feasible trade-abstain archetype winner-bank artifact. see $FAMILY_DIR/no_support_trade_abstain_archetype_winner_bank_summary.json" >&2
  exit 42
fi

echo "[ok] trade-abstain archetype winner-bank solve complete"
echo "runId=$RUN_ID"
