...
DEFAULT_RUN_ID="perfect_proto_stepb_1d_tp12_top_subscopes_probe200k_v1_$(date +%Y%m%d_%H%M%S)"
...
SUBSCOPE_IDS="TOP_RECENT_GAP_TOPHIGH,TOP_RECENT_CLOSE_ABOVE,TOP_RECENT_JUMP_ABOVE,TOP_RECENT_CROWDING_HIGH,TOP_RECENT_CROWDING_LOWMID"
...
usage() {
  cat <<EOF2
Usage: bash tools/server_run_stepb_1d_tp12_top_subscopes_200k.sh [--run-id=<run_id>] [--source-run-id=<broad_control_run>] [--subscope-ids=TOP_RECENT_GAP_TOPHIGH,...] [--promotable-first=true|false]
Reuses the existing broad 1D TP12 baseline control packs and reruns exact mining on filtered TOP recent sub-scopes with:
...
OUT_DIR="$PARENT_RUN_DIR/step-perfect-prototype-1d-tp12-top-subscopes-report"
mkdir -p "$OUT_DIR"

subscope_slug() {
  local subscope_id="$1"
  case "$subscope_id" in
    TOP_RECENT_GAP_TOPHIGH) printf 'gap_tophigh' ;;
    TOP_RECENT_CLOSE_ABOVE) printf 'close_above' ;;
    TOP_RECENT_JUMP_ABOVE) printf 'jump_above' ;;
    TOP_RECENT_CROWDING_HIGH) printf 'crowding_high' ;;
    TOP_RECENT_CROWDING_LOWMID) printf 'crowding_lowmid' ;;
...
subscope_line_id() {
  local subscope_id="$1"
  case "$subscope_id" in
    TOP_RECENT_GAP_TOPHIGH) printf 'stepb_dplus1_plus_lite_target12_top_gap_tophigh_probe' ;;
    TOP_RECENT_CLOSE_ABOVE) printf 'stepb_dplus1_plus_lite_target12_top_close_above_probe' ;;
    TOP_RECENT_JUMP_ABOVE) printf 'stepb_dplus1_plus_lite_target12_top_jump_above_probe' ;;
    TOP_RECENT_CROWDING_HIGH) printf 'stepb_dplus1_plus_lite_target12_top_crowding_high_probe' ;;
    TOP_RECENT_CROWDING_LOWMID) printf 'stepb_dplus1_plus_lite_target12_top_crowding_lowmid_probe' ;;
    *) printf 'stepb_dplus1_plus_lite_target12_top_%s_probe' "$(subscope_slug "$subscope_id")" ;;
  esac
}
...
run_subscope() {
  ...
  node tools/build_perfect_prototype_tp12_top_subscope_filtered_pack.mjs \
...
DONE_LINES=(
  "[done] 1D TP12 TOP-subscope probes completed"
  "  parentRunId=$RUN_ID"
...
for subscope_id in $(printf '%s' "$SUBSCOPE_IDS" | tr ',' ' '); do
  ...
  case "$subscope_id" in
    TOP_RECENT_GAP_TOPHIGH) REPORT_ARGS+=(--gap-tophigh="${scope_run_id}:${exit_code}:${wall_sec}") ;;
    TOP_RECENT_CLOSE_ABOVE) REPORT_ARGS+=(--close-above="${scope_run_id}:${exit_code}:${wall_sec}") ;;
    TOP_RECENT_JUMP_ABOVE) REPORT_ARGS+=(--jump-above="${scope_run_id}:${exit_code}:${wall_sec}") ;;
    TOP_RECENT_CROWDING_HIGH) REPORT_ARGS+=(--crowding-high="${scope_run_id}:${exit_code}:${wall_sec}") ;;
    TOP_RECENT_CROWDING_LOWMID) REPORT_ARGS+=(--crowding-lowmid="${scope_run_id}:${exit_code}:${wall_sec}") ;;
