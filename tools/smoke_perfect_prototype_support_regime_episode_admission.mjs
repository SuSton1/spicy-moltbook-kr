...
let family = {
  ...
  oosRows,
  supportCaseViews,
  bridgePositiveRows: gatedTrainRows.filter((row) => row.outcomeHitTarget === true),
  supportNearHardNegativeRows: gatedTrainRows.filter(
...
family = buildPerfectPrototypeSupportRegimeEpisodeDataset({ family, lookbackTradingDays: 4 })
family = buildPerfectPrototypeSupportEpisodeAdmissionFeatures({ family })

assert.equal(family.ok, true)
assert.equal(family.supportSymbolExcluded, true)
assert.ok((family.summary?.episodeAdmissionFeatureCount ?? 0) > 0)
...
assert.equal(admission.supportProjectionPositive, true)
assert.ok(admission.trainSummary.trainMatchedDateCount >= 10)
...
assert.equal(family.ok, true)
assert.ok((family.summary?.admittedTrainDateCount ?? 0) >= 10)

const solution = calibratePerfectPrototypeSupportAdmittedTop1Ranker({
  family,
  minTrainMatchedDates: 10,
  minTrainMatchedMonths: 6,
  minTrainMatchedFolds: 4,
...
assert.ok((solution.selectedFeaturePoolCount ?? 0) > 0)
assert.ok((solution.candidateCount ?? 0) > 0)
assert.ok(Array.isArray(solution.candidatesEvaluated))

console.log("ok: support regime episode admission")
