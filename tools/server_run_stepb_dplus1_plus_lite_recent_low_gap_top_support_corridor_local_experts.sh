...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_LOW_GAP_TOP_CORRIDOR_LOCAL_EXPERTS_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_low_gap_top_multibasin_simplex_complement_frontier_v42}_$(date +%Y%m%d_%H%M%S)"
...
EXPERT_DIR="$RUN_DIR/step-perfect-prototype-support-corridor-local-experts"

node "$ROOT_DIR/tools/build_perfect_prototype_low_gap_top_support_corridor_local_experts.mjs" \
...
if [[ -f "$EXPERT_DIR/no_support_corridor_local_experts_summary.json" ]]; then
  echo "[fatal] no feasible low-gap-top support corridor local experts. see $EXPERT_DIR/no_support_corridor_local_experts_summary.json" >&2
  exit 42
fi

echo "[ok] low-gap-top support corridor local experts solve complete"
echo "runId=$RUN_ID"
