...
import { buildPerfectPrototypeSupportFailureRegimeVetoTop1Unsat } from "../src/lib/perfect_prototype_support_failure_regime_veto_top1_unsat.mjs"

const usage = () => {
  console.error(`Usage:
  node tools/build_perfect_prototype_support_failure_regime_veto_top1.mjs \\
...
const parseArgs = (argv) => {
  const args = {
    familyId: "support_failure_regime_veto_top1",
...
const writeUnsat = async ({ outDir, value }) =>
  writeJson(path.join(outDir, "no_support_failure_regime_veto_top1_summary.json"), value)
...
const main = async () => {
  ...
  await writeJson(path.join(args.outDir, "failure_regime_veto_dataset_summary.json"), family.summary)
  if (family.ok !== true) {
  ...
  family = buildPerfectPrototypeSupportTemporalEpisodeDataset({ family, lookbackTradingDays: args.lookbackTradingDays })
  ...
  family = buildPerfectPrototypeSupportMatchedControlPool({ family, controlPoolSize: args.controlPoolSize })
  ...
  await writeJson(path.join(args.outDir, "support_failure_regime_corpus_summary.json"), family.summary)
  if (family.ok !== true) {
    await writeUnsat({ outDir: args.outDir, value: { ok: false, reason: family.reason ?? "unsat_no_failure_regime_corpus", supportFitExcluded: family.supportFitExcluded === true, summary: family.summary } })
    return
  }

  family = buildPerfectPrototypeSupportNegativeRegimeCluster({ family })
  await writeJson(path.join(args.outDir, "support_negative_regime_cluster_summary.json"), family.summary)
  if (family.ok !== true) {
    await writeUnsat({ outDir: args.outDir, value: { ok: false, reason: family.reason ?? "unsat_no_negative_regime_clusters", supportFitExcluded: family.supportFitExcluded === true, summary: family.summary } })
    return
  }

  family = buildPerfectPrototypeSupportNegativeRegimeVetoFeatures({ family })
  await writeJson(path.join(args.outDir, "support_negative_regime_veto_feature_summary.json"), family.summary)
  if (family.ok !== true) {
    await writeUnsat({ outDir: args.outDir, value: { ok: false, reason: family.reason ?? "unsat_no_negative_regime_veto_features", supportFitExcluded: family.supportFitExcluded === true, summary: family.summary } })
    return
  }
  ...
  await writeJson(path.join(args.outDir, "support_failure_regime_veto_summary.json"), veto)
  if (!veto.ok) {
    await writeUnsat({ outDir: args.outDir, value: buildPerfectPrototypeSupportFailureRegimeVetoTop1Unsat({ vetoResult: veto }) })
    return
  }
  ...
  await writeJson(path.join(args.outDir, "support_veto_survivor_query_summary.json"), family.summary)
  if (family.ok !== true || (family.supportCaseViews ?? []).length < 1) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportFailureRegimeVetoTop1Unsat({
        vetoResult: {
          ...veto,
          reason: family.ok !== true ? family.reason ?? "unsat_no_veto_survivors" : "unsat_veto_support_projection",
        },
      }),
    })
    return
  ...
  const ranker = calibratePerfectPrototypeSupportVetoSurvivorTop1Ranker({
    family,
    minTrainMatchedDates: args.minTrainDates,
    minTrainMatchedMonths: args.minTrainMonths,
    minTrainMatchedFolds: args.minTrainFolds,
  ...
  await writeJson(path.join(args.outDir, "support_veto_survivor_top1_summary.json"), ranker)
  if (!ranker.ok) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportFailureRegimeVetoTop1Unsat({ vetoResult: veto, ranker }),
    })
    return
  }
  ...
  await writeJson(path.join(args.outDir, "support_failure_regime_veto_top1_artifact.json"), artifact)
  await writeJson(path.join(args.outDir, "support_failure_regime_veto_top1_solution.json"), {
    ...ranker,
    vetoSummary: veto,
    artifact,
  })
