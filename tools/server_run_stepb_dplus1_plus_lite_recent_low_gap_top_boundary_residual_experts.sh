...
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_low_gap_top_grouped_boundary_residual_experts_v38}_$(date +%Y%m%d_%H%M%S)"
...
EXPERT_DIR="$RUN_DIR/step-perfect-prototype-boundary-residual-experts"

node "$ROOT_DIR/tools/build_perfect_prototype_low_gap_top_boundary_residual_experts.mjs" \
...
if [[ -f "$EXPERT_DIR/no_support_boundary_residual_experts_summary.json" ]]; then
  echo "[fatal] no feasible low-gap-top boundary residual experts. see $EXPERT_DIR/no_support_boundary_residual_experts_summary.json" >&2
  exit 42
fi

echo "[ok] low-gap-top boundary residual experts solve complete"
echo "runId=$RUN_ID"
