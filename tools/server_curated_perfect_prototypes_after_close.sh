...
DEFAULT_DATE="$(date +%F)"
DEFAULT_RUN_ID="perfect_proto_live_after_close_${DEFAULT_DATE//-/}_$(date +%H%M%S)"
...
usage() {
  cat <<EOF
Usage: bash tools/server_curated_perfect_prototypes_after_close.sh --catalog=/abs/path/catalog.json --expected-catalog-sha256=<sha256> --expected-rule-ids-sha256=<sha256> [--date=YYYY-MM-DD] [--config=/abs/path/config.json] [--run-id=<run_id>] [--allow-mutable-catalog=true]
EOF
}
...
if [[ -z "$CATALOG_PATH" ]]; then
  echo "[fatal] missing --catalog. Live apply now requires an explicit frozen catalog path." >&2
...
if [[ "$ALLOW_MUTABLE_CATALOG" != "true" && "$CATALOG_PATH" != *"/artifacts/curated/frozen/"*"/catalog.json" ]]; then
  echo "[fatal] live apply requires an immutable frozen catalog path under artifacts/curated/frozen/" >&2
...
const allParentRules = rules.every((rule) => String(rule?.ruleLevel ?? "").trim().toLowerCase() === "parent")
if (!allParentRules) {
  const levels = Array.from(new Set(rules.map((rule) => String(rule?.ruleLevel ?? "").trim() || "unknown"))).sort()
  throw new Error(
    [
      "server_curated_perfect_prototypes_after_close.sh only supports parent/daily-pack live catalogs.",
      `Found rule levels: ${levels.join(", ")}`,
      "Build a parent-compatible curated catalog or use a separate Step-B live apply path.",
    ].join(" "),
  )
}
NODE

node tools/build_perfect_prototype_daily_pack.mjs \
  --config="$CONFIG_PATH" \
  --out-run-id="$RUN_ID" \
  --start="$TARGET_DATE" \
  --end="$TARGET_DATE"
...
node tools/apply_perfect_prototypes.mjs \
  --input="$ROOT_DIR/artifacts/runs/$RUN_ID/step-perfect-prototype-daily/daily_pack.jsonl" \
...
echo "[ok] after-close run complete"
echo "runId=$RUN_ID"
echo "catalog=$CATALOG_PATH"
echo "dailyPack=$ROOT_DIR/artifacts/runs/$RUN_ID/step-perfect-prototype-daily/daily_pack.jsonl"
