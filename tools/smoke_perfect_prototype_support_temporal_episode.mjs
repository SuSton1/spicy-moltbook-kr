...
const episodeFamily = buildPerfectPrototypeSupportTemporalEpisodeDataset({ family, lookbackTradingDays: 4 })
assert.equal(episodeFamily.ok, true)
assert.ok((episodeFamily.summary?.episodeFeatureCount ?? 0) > 0)

const roleFamily = buildPerfectPrototypeSupportBagRoleTopologyFeatures({ family: episodeFamily })
assert.equal(roleFamily.ok, true)
assert.ok((roleFamily.summary?.roleTopologyFeatureCount ?? 0) > 0)

const solution = calibratePerfectPrototypeSupportTemporalAnchorScorer({
  family: roleFamily,
...
console.log("ok: support temporal episode")
