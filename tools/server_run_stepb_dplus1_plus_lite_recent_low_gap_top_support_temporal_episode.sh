...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_LOW_GAP_TOP_TEMPORAL_EPISODE_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_low_gap_top_temporal_episode_role_topology_scorer_v44}_$(date +%Y%m%d_%H%M%S)"
...
TEMPORAL_DIR="$RUN_DIR/step-perfect-prototype-support-temporal-episode"

node "$ROOT_DIR/tools/build_perfect_prototype_low_gap_top_support_temporal_episode.mjs" \
  --train-input="$TRAIN_INPUT_PATH" \
  --oos-input="$OOS_INPUT_PATH" \
  --support-cases-file="$SUPPORT_CASES_FILE" \
  --out-dir="$TEMPORAL_DIR" \
...
if [[ -f "$TEMPORAL_DIR/no_support_temporal_episode_summary.json" ]]; then
  echo "[fatal] no feasible low-gap-top support temporal episode scorer. see $TEMPORAL_DIR/no_support_temporal_episode_summary.json" >&2
  exit 42
fi

echo "[ok] low-gap-top support temporal episode scorer solve complete"
echo "runId=$RUN_ID"
echo "temporalDir=$TEMPORAL_DIR"
