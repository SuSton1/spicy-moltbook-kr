  ...
  const completionPromise = new Promise((resolve, reject) => {
    ...
    proc.once("close", (code) => {
      ...
      reject(
        new Error(
          [
            "Predictive indexed budget request/ack smoke worker failed",
            `script=${scriptPath}`,
...
const waitForJson = async (
  filePath,
  { timeoutMs = 10000, pollMs = 25, childProcess = null } = {},
) => {
  ...
  while (Date.now() - startedAt <= timeoutMs) {
    ...
    if (childProcess && childProcess.exitCode != null) {
      throw new Error(
        [
          `Worker exited before JSON file was written: ${filePath}`,
          `exitCode=${Number(childProcess.exitCode ?? 1)}`,
        ].join("\n"),
      )
    }
    await sleep(pollMs)
...
const main = async () => {
  ...
  const serverPolicy = assertPerfectPrototypeServerWorkspace({
    cwd,
    toolName: "smoke_prejump_parallel_budget_request_ack",
  })
  ...
  if (!rootDir) {
    throw new Error(
      "Parallel budget request/ack smoke missing rootDir from indexed adversarial smoke output",
    )
  }
  assertPerfectPrototypeServerPaths({
  ...
  const outDir = path.join(rootDir, "budget_request_ack_worker")
  ...
  const worker = spawnNodeCapture({
    ...
    args: [
      ...
      "--external-budget-decision-poll-ms=25",
  ...
  await writeJsonAtomic(budgetDecisionPath, {
    version: 1,
    requestRevision: Number(budgetRequest?.revision ?? 0),
    chunkIndex: workerChunkIndex,
    decision: "granted",
  ...
  assert(summary, "Parallel budget request/ack smoke missing worker summary.json")
  ...
  assert(Number(rejectionSummary?.budgetStopCount ?? 0) === 0)
  ...
  assert(
    Number(rejectionSummary?.externalAllocatedMaxSearchStates ?? 0) >
      Number(rejectionSummary?.allocatedMaxSearchStates ?? 0),
  )
