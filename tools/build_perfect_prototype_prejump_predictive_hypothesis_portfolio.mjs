...
const usage = () => {
  console.error(`Usage:
  node tools/build_perfect_prototype_prejump_predictive_hypothesis_portfolio.mjs \\
...
const PORTFOLIO_INPUT_NUMERIC_FEATURE_KEYS = [
  "seq40.mean",
  "seq40.delta",
  "seq40.last",
  "seq40.positiveShare",
...
const compactNumericFeatureMap = (numericFeatureMap) => {
  ...
  const out = {}
  for (const featureKey of PORTFOLIO_INPUT_NUMERIC_FEATURE_KEYS) {
...
const compactPrejumpPortfolioRow = (row) => {
...
const loadJsonl = async (filePath) =>
  readJsonl(filePath, {
    strict: true,
    map: (row) => compactPrejumpPortfolioRow(row),
  })
...
const writeUnsat = async ({ outDir, value }) =>
  writeJson(path.join(outDir, "no_support_prejump_predictive_hypothesis_portfolio_summary.json"), value)
...
const main = async () => {
  ...
  await writeJson(path.join(args.outDir, "prejump_portfolio_dataset_summary.json"), family.summary)
  if (family.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: {
        ok: false,
        reason: family.reason ?? "unsat_no_prejump_portfolio_dataset",
  ...
  await writeJson(path.join(args.outDir, "prejump_portfolio_feature_summary.json"), family.summary)
  if (family.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: {
        ok: false,
        reason: family.reason ?? "unsat_no_prejump_portfolio_features",
        supportFitExcluded: family.supportFitExcluded === true,
        summary: family.summary,
      },
    })
  ...
  await writeJson(path.join(args.outDir, "prejump_portfolio_audit_summary.json"), audit)
  await writeJson(path.join(args.outDir, "prejump_portfolio_hypothesis_reports.json"), audit.hypothesisReports ?? [])
  if (!audit.ok) {
    await writeUnsat({
      outDir: args.outDir,
      value: {
        ok: false,
        reason: audit.reason ?? "unsat_prejump_portfolio_audit",
        supportFitExcluded: family.supportFitExcluded === true,
  ...
  family = {
    ...family,
    portfolioSelectedHypothesisId: audit.selectedHypothesisId ?? null,
  ...
  await writeJson(path.join(args.outDir, "prejump_portfolio_top1_summary.json"), ranker)
  if (!ranker.ok) {
    await writeUnsat({
      outDir: args.outDir,
      value: {
        ...ranker,
        reason: ranker.reason ?? "unsat_prejump_portfolio_top1",
        selectedHypothesisId: audit.selectedHypothesisId ?? null,
      },
    })
    return
  }

  await writeJson(path.join(args.outDir, "prejump_portfolio_artifact.json"), ranker.artifact)
  await writeJson(path.join(args.outDir, "prejump_portfolio_solution.json"), {
    ...ranker,
    auditSummary: audit,
  })
}
