...
const main = async () => {
  ...
  if (!args["episodes-path"] || !args["delta-atoms-path"] || !args.out || !args["summary-out"]) {
    throw new Error("Usage: node tools/build_technique_episode_slice_contracts.mjs --episodes-path=<episode_windows.jsonl> --delta-atoms-path=<delta_atoms.jsonl> --out=<slice_contracts.jsonl> --summary-out=<summary.json> [--contract-path=PATH]")
  }
  ...
  const deltaRows = await readJsonl(path.resolve(cwd, String(args["delta-atoms-path"])), { strict: true })
  const report = searchTechniqueEpisodeSliceContracts({ episodeRows, deltaRows, contract })
  await writeJsonl(path.resolve(cwd, String(args.out)), report.sliceContracts)
