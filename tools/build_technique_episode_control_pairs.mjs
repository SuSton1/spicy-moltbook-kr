...
const main = async () => {
  ...
  if (!args["episodes-path"] || !args.out || !args["summary-out"]) {
    throw new Error("Usage: node tools/build_technique_episode_control_pairs.mjs --episodes-path=<episode_windows.jsonl> --out=<pairs.jsonl> --summary-out=<summary.json> [--contract-path=PATH]")
  }
  ...
  const dataset = buildTechniqueEpisodeControlPairs({ episodeRows, contract })
  await writeJsonl(path.resolve(cwd, String(args.out)), dataset.pairRows)
