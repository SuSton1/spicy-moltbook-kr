...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_SUPPORT_REGIME_EPISODE_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_support_counterfactual_episode_transition_admission_top1_v47}_$(date +%Y%m%d_%H%M%S)"
...
REGIME_DIR="$RUN_DIR/step-perfect-prototype-support-regime-episode-admission"

node "$ROOT_DIR/tools/build_perfect_prototype_support_regime_episode_admission.mjs" \
...
if [[ -f "$REGIME_DIR/no_support_regime_episode_summary.json" ]]; then
  echo "[fatal] no feasible support-regime episode admission artifact. see $REGIME_DIR/no_support_regime_episode_summary.json" >&2
  exit 42
fi

echo "[ok] support-regime episode admission solve complete"
echo "runId=$RUN_ID"
echo "regimeDir=$REGIME_DIR"
