...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_LOW_GAP_TOP_SUPPORT_SCORECARD_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_low_gap_top_support_scorecard_router_v31}_$(date +%Y%m%d_%H%M%S)"
...
SCORECARD_DIR="$RUN_DIR/step-perfect-prototype-support-scorecard"

node "$ROOT_DIR/tools/build_perfect_prototype_low_gap_top_support_scorecard.mjs" \
  --train-input="$TRAIN_INPUT_PATH" \
  --oos-input="$OOS_INPUT_PATH" \
  --support-cases-file="$DEFAULT_SUPPORT_CASES_FILE" \
  --out-dir="$SCORECARD_DIR" \
...
if [[ -f "$SCORECARD_DIR/no_support_scorecard_summary.json" ]]; then
  echo "[fatal] no feasible low-gap-top support scorecard. see $SCORECARD_DIR/no_support_scorecard_summary.json" >&2
  exit 42
fi

node "$ROOT_DIR/tools/report_perfect_prototype_low_gap_top_support_scorecard_oos.mjs" \
  --artifact="$SCORECARD_DIR/support_scorecard_artifact.json" \
  --input="$TRAIN_INPUT_PATH" \
  --support-cases-file="$DEFAULT_SUPPORT_CASES_FILE" \
  --out="$SCORECARD_DIR/support_scorecard_train_report.json"

node "$ROOT_DIR/tools/report_perfect_prototype_low_gap_top_support_scorecard_oos.mjs" \
  --artifact="$SCORECARD_DIR/support_scorecard_artifact.json" \
  --input="$OOS_INPUT_PATH" \
  --support-cases-file="$DEFAULT_SUPPORT_CASES_FILE" \
  --out="$SCORECARD_DIR/support_scorecard_oos_report.json"

echo "[ok] low-gap-top support scorecard solve complete"
echo "runId=$RUN_ID"
echo "scorecardDir=$SCORECARD_DIR"
