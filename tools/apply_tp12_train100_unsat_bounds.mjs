...
export const main = async (argv = process.argv.slice(2), { cwd = process.cwd() } = {}) => {
  ...
  if (!contractPath) throw new Error("apply_tp12_train100_unsat_bounds requires --contract")
  const summary = await applyTp12Train100UnsatBounds({
    contractPath: path.resolve(cwd, contractPath),
    frontierPath: toText(getFlag(flags, "frontier", "")),
    dominanceSummaryPath: toText(getFlag(flags, "dominance-summary", "")),
    ...
    JSON.stringify(
      {
        ...
        certifiedPrunedCount: summary.certifiedPrunedCount,
