...
usage() {
  cat <<'EOF'
Usage: bash tools/run_stepb_1d_tp12_no_gap_addon_rolling.sh --contract-path=PATH --addon-contract-path=PATH --run-id=<run_id> [--window-group=screen|final|all] [--window-ids=w1,w2] [--candle-path=PATH] [--max-rules=N]
Server-only orchestrator for TP12 no-gap addon rolling runs under a fixed lookback winner.
EOF
}
...
for window_id in "${window_ids_array[@]}"; do
  ...
  window_args=(
    ...
    "--addon-contract-path=${ADDON_CONTRACT_PATH}"
    "--window-id=${window_id}"
  ...
  bash tools/run_stepb_1d_tp12_no_gap_addon_window.sh "${window_args[@]}"
...
node --input-type=module - "$MANIFEST_PATH" "$RUN_ID" "$WINDOW_GROUP" "$RESOLVED_CONTRACT_PATH" "$MANIFEST_WINDOWS_JSONL" "$WINDOW_ID_LIST" "$MAX_RULES" "$CANDIDATE_ID" <<'NODE'
...
await writeJson(manifestPath, {
  kind: "tp12_no_gap_addon_rolling_manifest_v1",
...
echo "[done] tp12 no-gap addon rolling completed candidateId=${CANDIDATE_ID} runId=${RUN_ID} summary=${SUMMARY_PATH}"
