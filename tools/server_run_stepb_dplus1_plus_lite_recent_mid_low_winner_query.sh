...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_RECENT_MID_LOW_WINNER_QUERY_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_recent_mid_low_same_date_winner_query_v48}_$(date +%Y%m%d_%H%M%S)"
...
WINNER_DIR="$RUN_DIR/step-perfect-prototype-recent-mid-low-winner-query"

node "$ROOT_DIR/tools/build_perfect_prototype_recent_mid_low_winner_query.mjs" \
...
if [[ -f "$WINNER_DIR/no_support_recent_mid_low_winner_query_summary.json" ]]; then
  echo "[fatal] no feasible recent_mid_low winner query artifact. see $WINNER_DIR/no_support_recent_mid_low_winner_query_summary.json" >&2
  exit 42
fi

echo "[ok] recent_mid_low winner query solve complete"
echo "runId=$RUN_ID"
echo "winnerDir=$WINNER_DIR"
