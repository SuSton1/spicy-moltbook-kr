...
usage() {
  cat <<'EOF'
Usage: bash tools/run_technique_cluster_bank_discovery_rolling.sh --run-id=<run_id> --plan-path=PATH [--contract-path=PATH] [--cluster-bank-ids=id1,id2] [--max-banks=N] [--window-group=screen|all] [--candle-path=PATH] [--max-rules=N]
Server-only helper that drives explicit cluster-bank discovery entries into the fixed TP12 no-stop rolling wrapper.
EOF
}
...
OUT_DIR="$ROOT_DIR/artifacts/runs/$RUN_ID/step-perfect-prototype-technique-cluster-bank-discovery"
...
if [[ -e "$OUT_DIR" ]]; then
  fatal "technique cluster-bank discovery out dir already exists: $OUT_DIR"
fi
...
while IFS=$'\t' read -r CLUSTER_BANK_ID BANK_ID MECHANISM_ID SCOPE_ID LOOKBACK_CANDIDATE_ID ENTRY_TYPE TOP_TEMPLATE_ID SHORTLISTED_TEMPLATE_COUNT || [[ -n "${CLUSTER_BANK_ID:-}" ]]; do
  ...
  node tools/build_technique_cluster_bank_discovery_candidate_contract.mjs \
...
node --input-type=module - "$MANIFEST_PATH" "$RUN_ID" "$PLAN_PATH" "$PLAN_CONTRACT_ID" "$WINDOW_GROUP" "$MANIFEST_BANKS_JSONL" "$CLUSTER_BANK_IDS" "$MAX_BANKS" <<'NODE'
import fs from "node:fs/promises"
import { writeJson } from "./src/lib/io.mjs"
import { TECHNIQUE_CLUSTER_BANK_DISCOVERY_MANIFEST_KIND } from "./src/lib/technique_cluster_bank_discovery_contract.mjs"

const [manifestPath, runId, planPath, planContractId, windowGroup, banksJsonlPath, requestedClusterBankIdsCsv, maxBanks] = process.argv.slice(2)
const lines = String(await fs.readFile(banksJsonlPath, "utf8")).split("\n").map((line) => line.trim()).filter(Boolean)
const banks = lines.map((line, index) => {
  try {
    return JSON.parse(line)
  } catch (error) {
    throw new Error(`Malformed technique cluster-bank discovery manifest entry at ${banksJsonlPath}:${index + 1}: ${error instanceof Error ? error.message : String(error)}`)
  }
})
await writeJson(manifestPath, {
  kind: TECHNIQUE_CLUSTER_BANK_DISCOVERY_MANIFEST_KIND,
...
echo "[done] technique cluster-bank discovery rolling completed runId=${RUN_ID} banks=${SELECTED_BANK_COUNT} summary=${SUMMARY_PATH}"
