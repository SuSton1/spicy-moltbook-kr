...
const main = async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tp12-top-subscopes-"))
  await writeVariantRun({
    cwd: tempRoot,
    runId: "gap_run",
    lineId: "stepb_dplus1_plus_lite_target12_top_gap_tophigh_probe",
  ...
  await writeVariantRun({
    cwd: tempRoot,
    runId: "close_run",
    lineId: "stepb_dplus1_plus_lite_target12_top_close_above_probe",
  ...
  await writeVariantRun({
    cwd: tempRoot,
    runId: "crowd_run",
    lineId: "stepb_dplus1_plus_lite_target12_top_crowding_high_probe",
  ...
  const outDir = path.join(tempRoot, "artifacts", "runs", "parent", "step-perfect-prototype-1d-tp12-top-subscopes-report")
  await runNode(
    [
      path.join(process.cwd(), "tools", "build_stepb_1d_tp12_top_subscopes_report.mjs"),
      "--run-id=parent",
      "--source-run-id=broad_control",
      "--gap-tophigh=gap_run:0:111",
      "--close-above=close_run:0:112",
      "--crowding-high=crowd_run:0:113",
      `--out-dir=${outDir}`,
    ],
    tempRoot,
  )
  const verdict = await readJson(path.join(outDir, "top_subscopes_verdict.json"), null)
  if (verdict?.recommendedNextSubscopeId !== "TOP_RECENT_GAP_TOPHIGH") {
    throw new Error("expected recommendedNextSubscopeId=TOP_RECENT_GAP_TOPHIGH")
  }
  if (!Array.isArray(verdict?.scopesWithOosPerfectDateFloor3) || verdict.scopesWithOosPerfectDateFloor3[0] !== "TOP_RECENT_GAP_TOPHIGH") {
    throw new Error("expected TOP_RECENT_GAP_TOPHIGH to be the only perfect>=3 scope")
  }
}
