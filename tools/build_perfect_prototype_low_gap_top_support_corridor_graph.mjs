...
import { buildPerfectPrototypeSupportGraphUnsat } from "../src/lib/perfect_prototype_support_graph_unsat.mjs"

const usage = () => {
  console.error(`Usage:
  node tools/build_perfect_prototype_low_gap_top_support_corridor_graph.mjs \\
...
const writeUnsat = async ({ outDir, value }) =>
  writeJson(path.join(outDir, "no_support_corridor_graph_summary.json"), value)
...
const main = async () => {
  ...
  if (bridgeFamily.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportGraphUnsat({
  ...
  const dataset = buildPerfectPrototypeSupportCorridorGraphDataset({
  ...
  await writeJson(path.join(args.outDir, "support_corridor_graph_dataset_summary.json"), dataset.summary)
  if (dataset.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportGraphUnsat({
        reason: dataset.reason ?? "unsat_no_positive_graph_seeds",
        reasonCounts: { [dataset.reason ?? "unsat_no_positive_graph_seeds"]: 1 },
        familySummary: dataset.summary,
        supportFitExcluded: dataset.supportFitExcluded === true,
        graphPositiveSeedCount: dataset.summary?.graphPositiveSeedCount,
        graphNegativeSeedCount: dataset.summary?.graphNegativeSeedCount,
      }),
    })
    return
  }

  const metricFamily = buildPerfectPrototypeSupportCorridorMetricLearning({
    family: dataset,
  })
  await writeJson(path.join(args.outDir, "support_corridor_metric_summary.json"), metricFamily.summary)
  if (metricFamily.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportGraphUnsat({
        reason: metricFamily.reason ?? "unsat_no_corridor_metric_features",
        reasonCounts: { [metricFamily.reason ?? "unsat_no_corridor_metric_features"]: 1 },
        familySummary: metricFamily.summary,
        supportFitExcluded: metricFamily.supportFitExcluded === true,
        graphPositiveSeedCount: metricFamily.summary?.graphPositiveSeedCount,
        graphNegativeSeedCount: metricFamily.summary?.graphNegativeSeedCount,
      }),
    })
    return
  }

  const graphFamily = buildPerfectPrototypeSupportCorridorGraphBuilder({
    family: metricFamily,
  })
  await writeJson(path.join(args.outDir, "support_corridor_graph_summary.json"), graphFamily.summary)
  if (graphFamily.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportGraphUnsat({
        reason: graphFamily.reason ?? "unsat_no_corridor_graph",
        reasonCounts: { [graphFamily.reason ?? "unsat_no_corridor_graph"]: 1 },
        familySummary: graphFamily.summary,
        supportFitExcluded: graphFamily.supportFitExcluded === true,
        graphPositiveSeedCount: graphFamily.summary?.graphPositiveSeedCount,
        graphNegativeSeedCount: graphFamily.summary?.graphNegativeSeedCount,
      }),
    })
    return
  }
  ...
  await writeJson(path.join(args.outDir, "support_corridor_graph_propagation_summary.json"), propagated.summary)
  if (propagated.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportGraphUnsat({
        reason: propagated.reason ?? "unsat_no_graph_nodes",
        reasonCounts: { [propagated.reason ?? "unsat_no_graph_nodes"]: 1 },
        familySummary: propagated.summary,
        supportFitExcluded: propagated.supportFitExcluded === true,
        graphPositiveSeedCount: propagated.summary?.graphPositiveSeedCount,
        graphNegativeSeedCount: propagated.summary?.graphNegativeSeedCount,
        graphNodeCount: propagated.summary?.graphNodeCount,
        graphEdgeCount: propagated.summary?.graphEdgeCount,
      }),
    })
    return
  }
  ...
  await writeJson(path.join(args.outDir, "support_corridor_graph_artifact.json"), solution.artifact)
  await writeJson(path.join(args.outDir, "support_corridor_graph_train_summary.json"), solution.trainSummary)
  await writeJson(path.join(args.outDir, "support_corridor_graph_oos_summary.json"), solution.oosSummary)
  await writeJson(path.join(args.outDir, "support_corridor_graph_solution_summary.json"), {
    ok: true,
    supportFitExcluded: solution.supportFitExcluded === true,
    supportLeaveOneOutRecovered: solution.supportLeaveOneOutRecovered === true,
    graphPositiveSeedCount: solution.graphPositiveSeedCount ?? 0,
    graphNegativeSeedCount: solution.graphNegativeSeedCount ?? 0,
    ...
    artifactPath: path.join(args.outDir, "support_corridor_graph_artifact.json"),
  })
}
