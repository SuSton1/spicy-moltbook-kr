...
const main = async () => {
  ...
  if (!args["recurrence-report-path"] || !args["events-path"] || !args.out) {
    throw new Error(
      "Usage: node tools/build_technique_cluster_bank_shortlist.mjs --recurrence-report-path=<report.json> --events-path=<events.jsonl> --out=<shortlist.json> [--contract-path=PATH] [--seed-templates-path=PATH] [--seed-ids=id1,id2]",
    )
  }
  ...
  const selectedSeedIds =
    seedIds.length > 0
      ? seedIds
      : uniqueSortedStrings(Array.isArray(recurrenceReport.selectedSeedIds) ? recurrenceReport.selectedSeedIds : [])
