...
OOS_OUT_DIR="${OOS_OUT_DIR:-artifacts/runs/${RUN_ID}/oos}"
...
SURVIVORS_PATH="${SURVIVORS_PATH:-${TRAIN_OUT_DIR}/global_cover/micro_split_global_final_survivors.jsonl}"
...
ENRICHED_PATH="${ENRICHED_PATH:-${OOS_OUT_DIR}/oos_operational_consensus_rows.jsonl}"
ENRICHED_SUMMARY_PATH="${ENRICHED_SUMMARY_PATH:-${OOS_OUT_DIR}/oos_operational_consensus_summary.json}"
REPLAY_DIR="${REPLAY_DIR:-${OOS_OUT_DIR}/global_oos_cover_replay}"
export NODE_OPTIONS="${NODE_OPTIONS:---max-old-space-size=6144}"

[[ -f "$CONTRACT_PATH" ]] || { echo "contract not found: $CONTRACT_PATH" >&2; exit 2; }
[[ -f "$SURVIVORS_PATH" ]] || { echo "global survivors not found: $SURVIVORS_PATH" >&2; exit 2; }
