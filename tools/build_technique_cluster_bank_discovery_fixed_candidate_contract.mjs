...
import {
  ...
  writeTechniqueClusterBankDiscoveryFixedContract,
} from "../src/lib/technique_cluster_bank_discovery_fixed_contract.mjs"
import {
...
const resolveArgs = (argv = process.argv.slice(2), { cwd = process.cwd() } = {}) => {
  ...
  if (!planPath || !clusterBankId || !outPath) {
    throw new Error(
      "build_technique_cluster_bank_discovery_fixed_candidate_contract requires --plan-path, --cluster-bank-id, and --out",
    )
  }
  return {
    cwd,
...
export const main = async (argv = process.argv.slice(2), { cwd = process.cwd() } = {}) => {
  ...
  const result = await writeTechniqueClusterBankDiscoveryFixedContract({
    techniqueContract,
    fixedContract,
    planArtifact,
