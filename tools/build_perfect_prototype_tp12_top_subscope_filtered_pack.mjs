...
const main = async () => {
  ...
  const policy = assertPerfectPrototypeServerWorkspace({
    cwd,
    toolName: "build_perfect_prototype_tp12_top_subscope_filtered_pack",
  })
  ...
  if (!inputPath || !outDir || !subscopeId) {
    throw new Error(
      "Usage: node tools/build_perfect_prototype_tp12_top_subscope_filtered_pack.mjs --input=<daily_pack.jsonl> --out-dir=<dir> --subscope-id=<TOP_RECENT_...> [--source-run-id=<run>] [--source-stage-label=train|oos] [--row-contract=open_eval_recent_impulse_1d] [--require-nonempty=true|false]",
    )
  }
  assertPerfectPrototypeServerPaths({
    entries: [
