...
usage() {
  cat <<'EOF'
Usage: bash tools/run_tp12_year2x8_live_like_replay.sh --run-id=<run_id> --final-summary-path=<final_summary.json> [--contract-path=PATH]
Server-only helper that replays a year2x8 final-confirm OOS one requested date at a time and verifies equivalence against the final batch OOS apply.
EOF
}
...
[[ -n "$FINAL_SUMMARY_PATH" ]] || fatal "--final-summary-path is required"

eval "$(
  node --input-type=module - "$FINAL_SUMMARY_PATH" <<'NODE'
import { readJson } from "./src/lib/io.mjs"

const [finalSummaryPath] = process.argv.slice(2)
const summary = await readJson(finalSummaryPath, null)
...
emit("RESOLVED_FINAL_SUMMARY_PATH", finalSummaryPath)
emit("SCOPE_ID", summary.scopeId)
emit("CANDIDATE_ID", summary.candidateId)
emit("SOURCE_RUN_ID", summary.sourceRunId)
emit("SCOPE_RUN_ID", summary.scopeRunId)
emit("SELECTION_MODE", summary.selectionMode)
emit("OOS_START", summary.oosDateFrom)
emit("OOS_END", summary.oosDateTo)
NODE
)"

BASE_SCOPE_RUN_DIR="$ROOT_DIR/artifacts/runs/$SCOPE_RUN_ID"
...
DAY_RUN_ROOT="$RUN_DIR/live_like_days"
DATE_LIST_FILE="$RUN_DIR/requested_oos_dates.txt"
SUMMARY_PATH="$RUN_DIR/live_like_replay_summary.json"
RUNTIME_SUMMARY_PATH="$RUN_DIR/live_like_replay_runtime_summary.json"
...
node tools/build_tp12_year2x8_live_like_replay_summary.mjs \
  "--contract-path=${CONTRACT_PATH}" \
  "--final-summary-path=${RESOLVED_FINAL_SUMMARY_PATH}" \
...
node --input-type=module - "$RUNTIME_SUMMARY_PATH" "$RUN_ID" "$CONTRACT_PATH" "$RESOLVED_FINAL_SUMMARY_PATH" "$SOURCE_RUN_ID" "$SCOPE_RUN_ID" "$REQUESTED_DATE_COUNT" "$elapsed_sec" <<'NODE'
import { writeJson } from "./src/lib/io.mjs"

const [outPath, runId, contractPath, finalSummaryPath, sourceRunId, scopeRunId, requestedDateCount, wallClockSec] =
...
echo "[done] tp12 year2x8 live-like replay completed runId=${RUN_ID} requestedDates=${REQUESTED_DATE_COUNT}"
