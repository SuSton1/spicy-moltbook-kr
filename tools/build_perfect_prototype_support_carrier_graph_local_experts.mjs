...
const usage = () => {
  console.error(`Usage:
  node tools/build_perfect_prototype_support_carrier_graph_local_experts.mjs \\
...
const writeUnsat = async ({ outDir, value }) =>
  writeJson(path.join(outDir, "no_support_carrier_graph_local_experts_summary.json"), value)
...
const main = async () => {
  ...
  const carrierFamily = buildPerfectPrototypeSupportCarrierGraphFamily({
    cohort: boundaryFamily,
  })
  await writeJson(path.join(args.outDir, "support_carrier_graph_summary.json"), carrierFamily.summary)
  if (carrierFamily.ok !== true) {
    await writeUnsat({
      outDir: args.outDir,
      value: buildPerfectPrototypeSupportBoundaryExpertUnsat({
        reason: carrierFamily.reason ?? "unsat_no_carrier_components",
        reasonCounts: { [carrierFamily.reason ?? "unsat_no_carrier_components"]: 1 },
        familySummary: carrierFamily.summary,
        supportFitExcluded: carrierFamily.supportFitExcluded === true,
      }),
    })
    return
  }
  ...
  await writeJson(path.join(args.outDir, "support_carrier_graph_local_experts_artifact.json"), solution.artifact)
  await writeJson(path.join(args.outDir, "support_carrier_graph_local_experts_train_summary.json"), solution.trainSummary)
  await writeJson(path.join(args.outDir, "support_carrier_graph_local_experts_oos_summary.json"), solution.oosSummary)
  await writeJson(path.join(args.outDir, "support_carrier_graph_local_experts_solution_summary.json"), {
    ok: true,
    supportFitExcluded: solution.supportFitExcluded === true,
    supportLeaveOneOutRecovered: solution.supportLeaveOneOutRecovered === true,
    carrierEligibleComponentCount: carrierFamily.summary?.carrierEligibleComponentCount ?? 0,
    supportCaseReachableComponentCount: carrierFamily.summary?.supportCaseReachableComponentCount ?? 0,
    ...
    artifactPath: path.join(args.outDir, "support_carrier_graph_local_experts_artifact.json"),
  })
}
