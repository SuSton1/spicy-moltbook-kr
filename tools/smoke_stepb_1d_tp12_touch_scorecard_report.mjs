...
const buildCatalog = () =>
  applyPerfectPrototypeCatalogFreezeMetadata({
    ...
    sourceRunId: "smoke_tp12_touch_scorecard",
    selectionMode: "union_all",
    selectionLineId: "stepb_dplus1_plus_lite_target12_low_gap_top_probe",
...
const main = async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tp12-touch-scorecard-"))
  ...
  const outDir = path.join(tempRoot, "artifacts", "runs", "parent", "step-perfect-prototype-1d-tp12-touch-scorecard-report")
  await Promise.all([
  ...
  const baseTokens = ["tag:scope:gap_top", "tag:shape:reclaim"]
  const trainRows = [
    buildPackRow({ rowKey: "t1", dateKey: "2020-11-27", symbol: "T001", tokens: [...baseTokens, "tag:extra:a"] }),
    buildPackRow({ rowKey: "t2", dateKey: "2021-02-01", symbol: "T002", tokens: [...baseTokens, "tag:extra:a"] }),
    buildPackRow({ rowKey: "t3", dateKey: "2021-08-02", symbol: "T003", tokens: [...baseTokens, "tag:extra:a"] }),
    buildPackRow({ rowKey: "t4", dateKey: "2021-12-03", symbol: "T004", tokens: [...baseTokens, "tag:extra:a"] }),
    buildPackRow({ rowKey: "t5", dateKey: "2022-04-11", symbol: "T005", tokens: [...baseTokens, "tag:extra:b"] }),
    buildPackRow({ rowKey: "t6", dateKey: "2022-12-13", symbol: "T006", tokens: [...baseTokens, "tag:extra:b"] }),
    buildPackRow({ rowKey: "t7", dateKey: "2023-05-02", symbol: "T007", tokens: [...baseTokens, "tag:extra:c"] }),
    buildPackRow({ rowKey: "t8", dateKey: "2023-08-16", symbol: "T008", tokens: [...baseTokens, "tag:extra:c"] }),
    buildPackRow({ rowKey: "t9", dateKey: "2024-01-03", symbol: "T009", tokens: [...baseTokens, "tag:extra:c"] }),
    buildPackRow({ rowKey: "t10", dateKey: "2024-12-20", symbol: "T010", tokens: [...baseTokens, "tag:extra:c"] }),
  ]
  const oosRows = [
    buildPackRow({ rowKey: "o1", dateKey: "2025-01-02", symbol: "O001", tokens: [...baseTokens, "tag:extra:a"] }),
    buildPackRow({ rowKey: "o2", dateKey: "2025-04-02", symbol: "O002", tokens: [...baseTokens, "tag:extra:b"] }),
    buildPackRow({ rowKey: "o3", dateKey: "2025-08-04", symbol: "O003", tokens: [...baseTokens, "tag:extra:c"] }),
    buildPackRow({ rowKey: "o4", dateKey: "2025-11-17", symbol: "O004", tokens: [...baseTokens, "tag:extra:c"] }),
  ]
  ...
  const candleRows = [
    ...
    ...buildNegativeSeries({ symbol: "O003", decisionDateKey: "2025-08-04", entryDateKey: "2025-08-05", followDateKey: "2025-08-06", exitDateKey: "2025-08-07" }),
  ...
  await runNode(
    [
      path.join(process.cwd(), "tools", "build_stepb_1d_tp12_touch_scorecard_report.mjs"),
      `--scope-run-id=${runId}`,
      ...
      `--max-donor-terms=3`,
      `--max-cluster-terms=2`,
      `--max-scorecard-candidate-terms=3`,
      `--max-selected-terms=3`,
      `--beam-size=4`,
      `--min-solution-train-precision=0.4`,
      `--min-solution-train-dates=4`,
      `--min-solution-train-months=4`,
      `--min-solution-train-folds=2`,
      `--min-solution-selected-rows=4`,
      `--max-solution-top1-date-hit-share=0.3`,
      `--min-verdict-oos-selected-rows=1`,
      `--min-verdict-oos-dates=2`,
    ],
    tempRoot,
    {
  ...
  const summary = await readJson(path.join(outDir, "touch_scorecard_summary.json"), null)
  const verdict = await readJson(path.join(outDir, "touch_scorecard_verdict.json"), null)
  assert.equal(summary?.donorManifest?.donorCount >= 1, true)
  assert.equal(summary?.termBank?.candidateTermCount >= 1, true)
  assert.equal(summary?.scorecardBundle?.ok, true)
  assert.equal(summary?.scorecardBundle?.termCount >= 1, true)
  assert.equal(summary?.scorecardBundle?.oosSummary?.selectedRowCount >= 1, true)
  assert.equal(typeof verdict?.code, "string")
  console.log("ok: smoke_stepb_1d_tp12_touch_scorecard_report")
}
