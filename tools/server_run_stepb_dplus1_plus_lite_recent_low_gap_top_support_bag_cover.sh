...
DEFAULT_SUPPORT_CASES_FILE="${PERFECT_PROTO_LOW_GAP_TOP_BAG_COVER_SUPPORT_CASES_FILE:-/home/moltook/apps/stockdesk-lab-lite/artifacts/support_cases/haesung_076610_20260318_low_gap_top_v30.json}"
DEFAULT_RUN_ID="${STEPB_PLUS_LITE_WRAPPER_RUN_ID_PREFIX:-perfect_proto_support_recurrence_motif_bag_cover_v43}_$(date +%Y%m%d_%H%M%S)"
...
BAG_COVER_DIR="$RUN_DIR/step-perfect-prototype-support-bag-cover"

node "$ROOT_DIR/tools/build_perfect_prototype_low_gap_top_support_bag_cover.mjs" \
  --train-input="$TRAIN_INPUT_PATH" \
  --oos-input="$OOS_INPUT_PATH" \
  --support-cases-file="$SUPPORT_CASES_FILE" \
  --out-dir="$BAG_COVER_DIR" \
...
if [[ -f "$BAG_COVER_DIR/no_support_bag_cover_summary.json" ]]; then
  echo "[fatal] no feasible low-gap-top support bag cover. see $BAG_COVER_DIR/no_support_bag_cover_summary.json" >&2
  exit 42
fi

echo "[ok] low-gap-top support bag cover solve complete"
echo "runId=$RUN_ID"
echo "bagCoverDir=$BAG_COVER_DIR"
