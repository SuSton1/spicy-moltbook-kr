...
DEFAULT_RUN_ID="perfect_proto_support_like_surge_episode_bank_v58_$(date +%Y%m%d_%H%M%S)"
...
FAMILY_DIR="$RUN_DIR/step-perfect-prototype-support-like-surge-episode-bank"
...
node "$ROOT_DIR/tools/build_perfect_prototype_support_like_surge_episode_bank.mjs" \
  --config="$TRAIN_CONFIG_PATH" \
  ...
  --out-dir="$FAMILY_DIR" \
  --family-id=support_like_surge_episode_bank \
  --target-universe-id=support_like_surge_episode_v1 \
  ...
  --max-positive-rows-per-date=2 \
  --max-same-date-negatives-per-date=2 \
  --max-hard-negatives-per-date=1 \
  --archetype-min-date-count=4 \
  --archetype-max-count=5 \
  --archetype-max-share=0.8 \
  "${EXTRA_ARGS[@]}"

if [[ -f "$FAMILY_DIR/no_support_support_like_surge_episode_bank_summary.json" ]]; then
  echo "[fatal] no feasible support-like surge detector artifact. see $FAMILY_DIR/no_support_support_like_surge_episode_bank_summary.json" >&2
  exit 42
fi

echo "[ok] support-like surge episode solve complete"
echo "runId=$RUN_ID"
