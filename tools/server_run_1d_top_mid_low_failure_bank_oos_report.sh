...
DEFAULT_CONFIG_PATH="${PERFECT_PROTO_1D_CELL_CONFIG_PATH:-config/lab.config.server.lite.stepb_dplus1_plus_lite_lane_local.json}"
DEFAULT_RUN_ID="perfect_proto_1d_top_mid_low_failure_bank_oos_report_v59a_$(date +%Y%m%d_%H%M%S)"
...
REPORT_DIR="$RUN_DIR/step-perfect-prototype-1d-top-mid-low-failure-bank-oos-report"
...
node "$ROOT_DIR/tools/build_perfect_prototype_1d_top_mid_low_failure_bank_oos_report.mjs" \
  --config="$TRAIN_CONFIG_PATH" \
...
echo "[ok] v59a 1D TOP/MID/LOW report complete"
echo "runId=$RUN_ID"
